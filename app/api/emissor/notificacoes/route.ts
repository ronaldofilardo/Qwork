  ...
  try {
    ...
    const notificacoesQuery = await query(
      ...
      SELECT
        ...
        COUNT(CASE WHEN a.status = 'concluida' OR a.status = 'concluido' THEN 1 END) as avaliacoes_concluidas,
      ...
      HAVING
        COUNT(a.id) > 0
        AND COUNT(a.id) = COUNT(CASE WHEN a.status = 'concluida' OR a.status = 'concluido' THEN 1 END)
        AND (l.id IS NULL)
