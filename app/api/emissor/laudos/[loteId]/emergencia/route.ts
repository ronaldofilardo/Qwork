...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[EMERGÊNCIA] Falha na emissão imediata do lote ${loteId}:`,
        err
      );
    ...
    // Emissão sucedeu, mas sem registro de laudo (race/concorrência)
    console.warn(
      `[EMERGÊNCIA] emitirLaudoImediato completou, mas nenhum laudo encontrado para lote ${loteId}`
    );
    return NextResponse.json({
      success: true,
      message:
