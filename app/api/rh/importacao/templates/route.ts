...
/** GET /api/rh/importacao/templates — lista os templates do usuário autenticado nesta clínica */
  ...
  try {
    ...
    const result = await query(
      `SELECT id, nome, mapeamentos, nivel_cargo_map, criado_em
       ...
       WHERE clinica_id = $1 AND criado_por_cpf = $2
       ORDER BY criado_em DESC`,
      [session.clinica_id, session.cpf]
    );
    ...
    console.error('[templates/rh] GET error:', err);
...
/** POST /api/rh/importacao/templates — cria um novo template para o usuário autenticado */
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO importacao_templates
         (nome, clinica_id, criado_por_cpf, mapeamentos, nivel_cargo_map)
       VALUES ($1, $2, $3, $4, $5)
    ...
    console.error('[templates/rh] POST error:', err);
    return NextResponse.json(
