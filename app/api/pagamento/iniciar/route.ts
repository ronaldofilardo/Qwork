  ...
  try {
    ...
    if (contratoIdParam) {
      contratanteResult = await query(
        `SELECT c.id, c.nome, c.status, COALESCE(c.numero_funcionarios_estimado, 1) as numero_funcionarios, 
    ...
    // Quando houver um contrato com status 'aguardando_pagamento' ou com `aceito = true`,
    // também permitimos iniciar o pagamento.
    ...
    // Usar dados fornecidos ou calcular baseado no contrato
