 ...
 *
 * Design System QWork:
 * - Verde primário #6DB33F | Cinza escuro #3D3D3D | Cinza médio #6E6E6E
 * - Tipografia: Helvetica Bold/Normal/Italic
 * - Capa com logo, cards de dados, rodapé de validade jurídica
 * - Páginas de contrato com header correndo, cláusulas estruturadas e separadores de linha
 * - Assinaturas com logo QWork ao final
 */
...
// ─── Design Tokens QWork ─────────────────────────────────────────────────────
const COR_PRIMARIA: [number, number, number] = [0, 0, 0]; // #000000 - Preto
const COR_ESCURA: [number, number, number] = [30, 30, 30]; // #1E1E1E - Escuro
const COR_MEDIA: [number, number, number] = [80, 80, 80]; // #505050 - Médio
const COR_CLARA: [number, number, number] = [220, 220, 220]; // #DCDCDC - Claro
const COR_BG_VERDE: [number, number, number] = [248, 248, 248]; // #F8F8F8 - Cinza ultra claro (antigo BG verde)
...
export async function GET() {
  try {
    ...
    if (session.perfil === 'gestor') {
      if (!session.entidade_id) {
        return NextResponse.json(
          { error: 'Entidade não identificada na sessão' },
          { status: 403 }
        );
      }
    ...
    if (!tomadorId || !tomadorData) {
      return NextResponse.json(
        { error: 'Tomador não identificado' },
        { status: 403 }
      );
    }
    ...
    // Removida a faixa verde do topo para fundo branco e logo preto (Design Clean)
    ...
    if (logoBase64) {
      ...
      doc.setFontSize(24);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...COR_PRIMARIA); // Preto
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...COR_ESCURA);
    doc.text('CONTRATO DE PRESTAÇÃO DE SERVIÇOS', PW / 2, y, {
      align: 'center',
    });
    y += 7;
    ...
    doc.setDrawColor(...COR_PRIMARIA); // Preto
    doc.setLineWidth(0.4);
    doc.roundedRect(M - 3, cardContrY - 4, CW + 6, cardContrH, 1, 1, 'S');
    doc.setFillColor(...COR_PRIMARIA); // Preto
    ...
    doc.addPage();
    // Reformatar abertura conforme pedido - Centralizar título em preto
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...COR_PRIMARIA); // Preto
    ...
    // Dados do Responsável e Aceite explicitamente solicitados
    doc.setFontSize(7.5);
    doc.setTextColor(...COR_ESCURA);
    doc.text(
