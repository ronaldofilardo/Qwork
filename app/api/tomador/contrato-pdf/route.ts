...
export async function GET() {
  try {
    ...
    // Buscar contrato ativo do tomador (entidade ou clínica)
    const result = await query(
      `SELECT
        ...
        c.data_aceite,
        c.ip_aceite,
        COALESCE(e.nome, cl.nome) as tomador_nome,
        COALESCE(e.cnpj, cl.cnpj) as tomador_cnpj,
        COALESCE(e.responsavel_nome, cl.responsavel_nome) as responsavel_nome,
        p.nome as plano_nome
      FROM contratos c
      LEFT JOIN entidades e ON c.entidade_id = e.id
      LEFT JOIN clinicas cl ON c.clinica_id = cl.id
      LEFT JOIN contratos_planos cp ON (
        (c.entidade_id IS NOT NULL AND cp.entidade_id = c.entidade_id) OR
        (c.clinica_id IS NOT NULL AND cp.clinica_id = c.clinica_id)
      )
      LEFT JOIN planos p ON cp.plano_id = p.id
      WHERE (c.entidade_id = $1 OR c.clinica_id = $1)
      AND c.aceito = true
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(`Tomador: ${contrato.tomador_nome}`, margin, yPosition);
    yPosition += 6;
    doc.text(`CNPJ: ${contrato.tomador_cnpj}`, margin, yPosition);
    yPosition += 6;
