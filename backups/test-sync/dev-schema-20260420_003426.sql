...
--
-- Name: modelo_comissionamento; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.modelo_comissionamento AS ENUM (
    'percentual',
    'custo_fixo'
);
...
--
-- Name: motivo_congelamento; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: status_cadastro_lead; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: status_comissao; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: status_lead; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: status_representante; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: status_vinculo; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: tipo_conversao_lead; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: tipo_pessoa_representante; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION atualizar_ultima_avaliacao_funcionario(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: audit_laudo_delete_attempt(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION audit_laudo_delete_attempt(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: audit_status_pagamento_change(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: calcular_elegibilidade_lote_tomador(integer, integer); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION calcular_elegibilidade_lote_tomador(p_tomador_id integer, p_numero_lote_atual integer); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: calcular_valor_total_lote(integer); Type: FUNCTION; Schema: public; Owner: -
--
...
BEGIN
    -- Billing sobre total liberadas (status != 'rascunho'), nÃ£o apenas concluÃ­das
    -- Ex: 100 liberadas + 70 concluÃ­das â†’ cobrar 100 * valor_por_funcionario
    SELECT
...
--
-- Name: FUNCTION calcular_valor_total_lote(p_lote_id integer); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.calcular_valor_total_lote(p_lote_id integer) IS 'Calcula valor total de cobranÃ§a do lote.
CobranÃ§a Ã© sobre total_liberadas (status != rascunho), nÃ£o apenas concluÃ­das.
PolÃ­tica: lote com 100 liberadas cobra 100 * valor, mesmo que sÃ³ 70 concluÃ­das.
Migration 1130: substituiu base de cÃ¡lculo de concluÃ­das para liberadas.';
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND OLD.emitido_em IS NOT NULL) THEN
    ...
    -- ExceÃ§Ã£o 1: Backfill do hash_pdf quando estava NULL ou vazio
    -- Permite definir hash_pdf retroativamente sem alterar nenhum outro campo
    IF (OLD.hash_pdf IS NULL OR OLD.hash_pdf = '') AND
       ...
       OLD.observacoes IS NOT DISTINCT FROM NEW.observacoes AND
       (OLD.emitido_em = NEW.emitido_em OR (OLD.emitido_em IS NULL AND NEW.emitido_em IS NULL)) AND
       (OLD.enviado_em IS NOT DISTINCT FROM NEW.enviado_em) THEN
      RETURN NEW;
    END IF;

    -- ExceÃ§Ã£o 2: Primeiro upload remoto para armazenamento em Backblaze/S3
    -- Permite atualizar campos arquivo_remoto_* e marcar como 'enviado'
    -- quando ainda nÃ£o havia arquivo remoto (OLD.arquivo_remoto_key IS NULL)
    -- Campos de integridade documental (lote_id, emissor_cpf, hash_pdf) NÃƒO podem mudar
    IF OLD.arquivo_remoto_key IS NULL AND
       NEW.arquivo_remoto_key IS NOT NULL AND
       OLD.lote_id = NEW.lote_id AND
       OLD.emissor_cpf = NEW.emissor_cpf AND
       OLD.hash_pdf IS NOT DISTINCT FROM NEW.hash_pdf AND
       OLD.observacoes IS NOT DISTINCT FROM NEW.observacoes THEN
      RETURN NEW;
    END IF;

    -- Qualquer outra tentativa de modificaÃ§Ã£o Ã© bloqueada
    RAISE EXCEPTION 'NÃ£o Ã© permitido modificar laudos jÃ¡ emitidos. Laudo ID: %', OLD.id
      USING HINT = 'Laudos emitidos sÃ£o imutÃ¡veis para garantir integridade documental.';
  END IF;

  -- DELETE nÃ£o Ã© permitido para laudos emitidos
  IF (TG_OP = 'DELETE' AND OLD.emitido_em IS NOT NULL) THEN
    RAISE EXCEPTION 'NÃ£o Ã© permitido deletar laudos jÃ¡ emitidos. Laudo ID: %', OLD.id
      USING HINT = 'Laudos emitidos sÃ£o imutÃ¡veis para garantir integridade documental.';
  END IF;
...
COMMENT ON FUNCTION public.check_laudo_immutability() IS 'Garante imutabilidade de laudos emitidos. ExceÃ§Ã£o 1: backfill hash_pdf quando NULL. ExceÃ§Ã£o 2: primeiro upload remoto (arquivo_remoto_key NULLâ†’NOT NULL) preservando campos de integridade (lote_id, emissor_cpf, hash_pdf, observacoes). Migration 1149.';
...
--
-- Name: current_representante_id(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION current_representante_id(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: fn_bloquear_campos_sensiveis_emissor(); Type: FUNCTION; Schema: public; Owner: -
--
...
BEGIN
    ...
    -- Aplicar restriÃ§Ã£o apenas ao perfil emissor
    IF v_perfil = 'emissor' THEN
        -- Proteger campos de conteÃºdo gerado pelo sistema de avaliaÃ§Ã£o
        IF (OLD.observacoes IS DISTINCT FROM NEW.observacoes) THEN
            RAISE EXCEPTION 'Emissor nÃ£o tem permissÃ£o para alterar o campo observacoes de um laudo.'
                USING ERRCODE = '42501', HINT = 'O campo observacoes Ã© gerenciado pelo sistema.';
        END IF;

        -- Proteger lote_id (nÃ£o pode reatribuir laudo a outro lote)
        IF (OLD.lote_id IS DISTINCT FROM NEW.lote_id) THEN
            RAISE EXCEPTION 'Emissor nÃ£o tem permissÃ£o para alterar o lote_id de um laudo.'
                USING ERRCODE = '42501', HINT = 'lote_id Ã© imutÃ¡vel apÃ³s criaÃ§Ã£o.';
        END IF;

        -- Proteger emissor_cpf apÃ³s definiÃ§Ã£o (anti-fraude: nÃ£o pode apropriar laudo alheio)
        IF (OLD.emissor_cpf IS NOT NULL AND OLD.emissor_cpf IS DISTINCT FROM NEW.emissor_cpf) THEN
            RAISE EXCEPTION 'Emissor nÃ£o tem permissÃ£o para alterar emissor_cpf apÃ³s definiÃ§Ã£o.'
                USING ERRCODE = '42501', HINT = 'emissor_cpf Ã© imutÃ¡vel apÃ³s atribuiÃ§Ã£o.';
        END IF;
    END IF;
...
--
-- Name: fn_cpf_em_uso(text); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION fn_cpf_em_uso(p_cpf text); Type: COMMENT; Schema: public; Owner: -
--
...
DECLARE
  ...
  v_max_retries int := 5;
BEGIN
  -- ReferÃªncia do MAX atual para evitar colisÃ£o com rows inseridas fora do allocator
  SELECT COALESCE(MAX(id), 0) INTO v_max_existing FROM lotes_avaliacao;

  LOOP
    -- Atualiza atomicamente; GREATEST garante nÃ£o regredir abaixo do MAX existente
    UPDATE lote_id_allocator
       SET last_id = GREATEST(last_id + 1, v_max_existing + 1)
    RETURNING last_id INTO v_next;

    -- NULL-guard: tabela estava vazia â€” inicializa e tenta mais 1 vez
    IF v_next IS NULL THEN
      IF v_retries > 0 THEN
        RAISE EXCEPTION '[fn_next_lote_id] lote_id_allocator vazia mesmo apÃ³s tentativa de inicializaÃ§Ã£o. '
                        'Execute: INSERT INTO lote_id_allocator (last_id) SELECT COALESCE(MAX(id),0) FROM lotes_avaliacao;';
      END IF;

      -- InicializaÃ§Ã£o de emergÃªncia (evita NULL silencioso)
    ...
    -- Verificar colisÃ£o com id jÃ¡ existente
    IF NOT EXISTS (SELECT 1 FROM lotes_avaliacao WHERE id = v_next) THEN
      RETURN v_next;
    END IF;

    -- ColisÃ£o: sincronizar e tentar novamente
    v_retries := v_retries + 1;
    IF v_retries >= v_max_retries THEN
      RAISE EXCEPTION '[fn_next_lote_id] Falha ao gerar ID Ãºnico apÃ³s % tentativas (Ãºltimo candidato: %)',
        v_max_retries, v_next;
    END IF;

    RAISE WARNING '[fn_next_lote_id] ColisÃ£o no ID %. Tentativa % de %', v_next, v_retries, v_max_retries;
...
--
-- Name: FUNCTION fn_next_lote_id(); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.fn_next_lote_id() IS 'Retorna prÃ³ximo ID para lotes_avaliacao de forma atÃ´mica.
Migration 1142 (05/04/2026): adicionado NULL-guard explÃ­cito, RAISE EXCEPTION descritivo
e inicializaÃ§Ã£o de emergÃªncia. GREATEST() previne colisÃ£o com INSERTs externos ao allocator.';
...
COMMENT ON FUNCTION public.fn_reservar_id_laudo_on_lote_insert() IS 'Reserva ID do laudo (igual ao lote) em status rascunho ao criar lote. Status rascunho permite criar sem hash_pdf, evitando erro de validaÃ§Ã£o.';
...
--
-- Name: gerar_codigo_representante(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION gerar_codigo_representante(); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.gerar_codigo_representante() IS 'Gera cÃ³digo numÃ©rico sequencial para representante (a partir de 100). CÃ³digos anteriores alfanumÃ©ricos permanecem.';
...
--
-- Name: gerar_token_lead(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION gerar_token_lead(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: job_auto_cancelar_comissoes_congeladas(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION job_auto_cancelar_comissoes_congeladas(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: job_encerrar_vinculos_expirados(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION job_encerrar_vinculos_expirados(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: job_expirar_leads_vencidos(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION job_expirar_leads_vencidos(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: job_marcar_vinculos_inativos(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION job_marcar_vinculos_inativos(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: liberar_comissoes_retidas(integer); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION liberar_comissoes_retidas(p_representante_id integer); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION prevent_gestor_being_emissor(); Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON FUNCTION public.prevent_gestor_being_emissor() IS 'Migration 411: Impede que CPF de gestor de entidade seja usado como emissor em funcionarios. Usa entidades/entidades_senhas (nomes atuais, substituindo contratantes legacy).';
...
--
-- Name: prevent_mutation_laudos_storage_log(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION prevent_mutation_laudos_storage_log(); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: registrar_auditoria_comissionamento(character varying, integer, character varying, character varying, character varying, text, jsonb, character); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION registrar_auditoria_comissionamento(p_tabela character varying, p_registro_id integer, p_status_ant character varying, p_status_novo character varying, p_triggador character varying, p_motivo text, p_dados_extras jsonb, p_cpf character); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: set_atualizado_em_comissionamento(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: set_hierarquia_comercial_updated_at(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: trg_auditar_comissao_status(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: trg_auditar_representante_status(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: trg_auditar_vinculo_status(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: trg_gerar_codigo_representante(); Type: FUNCTION; Schema: public; Owner: -
--
...
COMMENT ON FUNCTION public.trg_reject_prohibited_roles_func() IS 'Migration 1206 (corrige 410): Usa NEW.perfil em vez de NEW.usuario_tipo. A tabela funcionarios tem coluna perfil, não usuario_tipo. Rejeita apenas gestor_entidade que pertence à tabela usuarios.';
...
--
-- Name: update_representantes_senhas_atualizado_em(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: update_vendedores_dados_bancarios_atualizado_em(); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: update_vendedores_perfil_atualizado_em(); Type: FUNCTION; Schema: public; Owner: -
--
...
BEGIN
  ...
  -- Contar avaliaÃ§Ãµes (base: liberadas = status != 'rascunho')
  SELECT
    COUNT(*) FILTER (WHERE status != 'rascunho')::int,
    COUNT(*) FILTER (WHERE status = 'concluida')::int,
  ...
  IF v_total_liberadas = 0 THEN
    v_alertas := array_append(v_alertas,
      'ERRO: Lote nÃ£o possui avaliaÃ§Ãµes liberadas. NÃ£o Ã© possÃ­vel gerar laudo.');
    v_bloqueante := TRUE;
  ...
  ELSIF v_avaliacoes_concluidas < v_threshold_70 THEN
    -- Abaixo de 70% â€” ainda nÃ£o elegÃ­vel
    v_alertas := array_append(v_alertas,
      'PENDENTE: ' || v_avaliacoes_concluidas || ' de ' || v_total_liberadas ||
      ' avaliaÃ§Ãµes concluÃ­das. NecessÃ¡rio pelo menos ' || v_threshold_70 ||
      ' (' || ROUND((v_threshold_70::NUMERIC / NULLIF(v_total_liberadas, 0)) * 100, 0) ||
      '% de ' || v_total_liberadas || ' liberadas). Atual: ' ||
      v_taxa_conclusao || '%.');
    v_bloqueante := TRUE;
  ELSE
    -- >=70% concluÃ­das: lote elegÃ­vel (deve estar em status 'concluido' via trigger)
    IF v_lote_status != 'concluido' AND v_lote_status != 'emissao_solicitada'
      ...
      v_alertas := array_append(v_alertas,
        ...
        ' avaliaÃ§Ãµes concluÃ­das (â‰¥70%), mas lote ainda estÃ¡ em status "' ||
        v_lote_status || '". Aguarde a atualizaÃ§Ã£o automÃ¡tica do sistema.');
      v_bloqueante := TRUE;
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.validar_lote_pre_laudo(p_lote_id integer) IS 'Valida se lote estÃ¡ pronto para emissÃ£o de laudo.
PolÃ­tica 70%: requer avaliacoes_concluidas >= CEIL(0.7 * total_liberadas).
Total_liberadas = COUNT WHERE status != rascunho (inclui inativadas na base).
Migration 1130: substituiu polÃ­tica 100% (concluidas + inativadas = total).';
...
--
-- Name: validar_token_pagamento(uuid); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: verificar_lead_ativo_por_cnpj(character); Type: FUNCTION; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION verificar_lead_ativo_por_cnpj(p_cnpj character); Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_entidade; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.aceites_termos_entidade (
    ...
    aceito_em timestamp without time zone DEFAULT now() NOT NULL,
    ip_address inet,
    responsavel_removido_em timestamp without time zone,
    responsavel_remover_motivo text
);


--
-- Name: TABLE aceites_termos_entidade; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.aceites_termos_entidade IS 'Registro de aceites vinculados ao CNPJ/contratante (prova legal mesmo após remoção do usuário)';


--
-- Name: COLUMN aceites_termos_entidade.entidade_cnpj; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN aceites_termos_entidade.responsavel_cpf; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN aceites_termos_entidade.responsavel_removido_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_entidade_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_entidade_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_usuario; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE aceites_termos_usuario; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.aceites_termos_usuario IS 'Registro individual de aceites de termos por CPF (auditoria legal/LGPD)';


--
-- Name: COLUMN aceites_termos_usuario.usuario_cpf; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN aceites_termos_usuario.usuario_tipo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN aceites_termos_usuario.termo_tipo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN aceites_termos_usuario.versao_termo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_usuario_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_usuario_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: audit_delecoes_tomador; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.audit_delecoes_tomador (
    id integer NOT NULL,
    ...
    admin_nome character varying(255) NOT NULL,
    resumo jsonb DEFAULT '{}'::jsonb NOT NULL,
    criado_em timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT audit_delecoes_tomador_tipo_check CHECK (((tipo)::text = ANY ((ARRAY['entidade'::character varying, 'clinica'::character varying])::text[])))
);


--
-- Name: TABLE audit_delecoes_tomador; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: audit_delecoes_tomador_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.audit_delecoes_tomador_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
--
-- Name: audit_delecoes_tomador_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.audit_delecoes_tomador_id_seq OWNED BY public.audit_delecoes_tomador.id;
...
CREATE TABLE public.avaliacoes (
    ...
    CONSTRAINT avaliacoes_status_check CHECK (((status)::text = ANY (ARRAY[('rascunho'::character varying)::text, ('iniciada'::character varying)::text, ('em_andamento'::character varying)::text, ('concluido'::character varying)::text, ('concluida'::character varying)::text, ('inativada'::character varying)::text])))
);
...
--
-- Name: COLUMN avaliacoes.identificacao_confirmada; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.avaliacoes.identificacao_confirmada IS 'Flag: funcionÃ¡rio confirmou ser o titular da avaliaÃ§Ã£o antes de responder';


--
-- Name: COLUMN avaliacoes.identificacao_confirmada_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.avaliacoes.identificacao_confirmada_em IS 'Timestamp em que a confirmaÃ§Ã£o de identificaÃ§Ã£o foi registrada';
...
ALTER TABLE ONLY public.clinica_configuracoes FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.clinicas (
    ...
    credito_manutencao_pendente numeric(10,2) DEFAULT 0.00 NOT NULL,
    isento_pagamento boolean DEFAULT false NOT NULL,
...
--
-- Name: COLUMN clinicas.credito_manutencao_pendente; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.clinicas.credito_manutencao_pendente IS 'Crédito acumulado de taxas de manutenção pagas por empresas desta clínica.';
...
--
-- Name: comissionamento_auditoria; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE comissionamento_auditoria; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: comissionamento_auditoria_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: comissionamento_auditoria_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.comissoes_laudo (
    ...
    valor_comissao_comercial numeric(15,2) DEFAULT 0 NOT NULL,
    asaas_payment_id character varying(50),
    asaas_split_executado boolean DEFAULT false NOT NULL,
    asaas_split_confirmado_em timestamp with time zone,
    ...
    CONSTRAINT comissoes_perc_comercial_range CHECK (((percentual_comissao_comercial >= (0)::numeric) AND (percentual_comissao_comercial <= (40)::numeric)))
);


--
-- Name: TABLE comissoes_laudo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN comissoes_laudo.percentual_comissao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.percentual_comissao IS 'Percentual de comissão aplicado (apenas representante).';


--
-- Name: COLUMN comissoes_laudo.valor_comissao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.valor_comissao IS 'Valor final da comissÃ£o: valor_laudo Ã— percentual_comissao / 100. Armazenado para auditabilidade e histÃ³rico.';


--
-- Name: COLUMN comissoes_laudo.mes_emissao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN comissoes_laudo.mes_pagamento; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN comissoes_laudo.lote_pagamento_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.lote_pagamento_id IS 'FK para o lote de avaliaÃ§Ã£o cujo pagamento originou esta comissÃ£o. Usado para prevenir duplicatas.';


--
-- Name: COLUMN comissoes_laudo.clinica_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.clinica_id IS 'FK para clinicas: preenchido quando o tomador Ã© uma clÃ­nica RH-flow sem entidade espelho. Mutuamente exclusivo com entidade_id.';


--
-- Name: COLUMN comissoes_laudo.parcela_numero; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN comissoes_laudo.total_parcelas; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN comissoes_laudo.parcela_confirmada_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN comissoes_laudo.arquivado; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.arquivado IS 'TRUE para registros legados (modelo anterior). NÃ£o aparecem na UI do novo modelo.';


--
-- Name: COLUMN comissoes_laudo.percentual_comissao_comercial; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.percentual_comissao_comercial IS 'Percentual de comissão do comercial aplicado nesta comissão.';


--
-- Name: COLUMN comissoes_laudo.valor_comissao_comercial; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.valor_comissao_comercial IS 'Valor em R$ da comissão do comercial para este laudo (baseCalculo × percentual_comissao_comercial / 100).';


--
-- Name: COLUMN comissoes_laudo.asaas_payment_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.asaas_payment_id IS 'ID do pagamento Asaas que originou esta comissÃ£o (para rastreio de split).';


--
-- Name: COLUMN comissoes_laudo.asaas_split_executado; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.asaas_split_executado IS 'Se o split Asaas foi executado na subconta do representante.';


--
-- Name: COLUMN comissoes_laudo.asaas_split_confirmado_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comissoes_laudo.asaas_split_confirmado_em IS 'Timestamp da confirmaÃ§Ã£o do split pelo webhook Asaas.';


--
-- Name: comissoes_laudo_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE confirmacao_identidade; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.confirmacao_identidade IS 'Registros de confirmaÃ§Ã£o de identidade para fins de auditoria jurÃ­dica';


--
-- Name: COLUMN confirmacao_identidade.avaliacao_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.avaliacao_id IS 'Foreign key para avaliacoes. Pode ser NULL para confirmaÃ§Ãµes de identidade feitas no contexto de login.';


--
-- Name: COLUMN confirmacao_identidade.funcionario_cpf; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.funcionario_cpf IS 'CPF do funcionÃ¡rio que confirmou';


--
-- Name: COLUMN confirmacao_identidade.nome_confirmado; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.nome_confirmado IS 'Nome exibido na confirmaÃ§Ã£o';


--
-- Name: COLUMN confirmacao_identidade.cpf_confirmado; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.cpf_confirmado IS 'CPF exibido na confirmaÃ§Ã£o (deve ser igual ao funcionario_cpf)';


--
-- Name: COLUMN confirmacao_identidade.data_nascimento; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.data_nascimento IS 'Data de nascimento exibida na confirmaÃ§Ã£o';


--
-- Name: COLUMN confirmacao_identidade.confirmado_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.confirmado_em IS 'Data/hora em que a confirmaÃ§Ã£o foi realizada';


--
-- Name: COLUMN confirmacao_identidade.ip_address; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.ip_address IS 'EndereÃ§o IP de origem da confirmaÃ§Ã£o';


--
-- Name: COLUMN confirmacao_identidade.user_agent; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.confirmacao_identidade.user_agent IS 'User-Agent do navegador usado na confirmaÃ§Ã£o';


--
-- Name: confirmacao_identidade_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: creditos_manutencao; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.creditos_manutencao (
    id integer NOT NULL,
    entidade_id integer,
    clinica_id integer,
    empresa_id integer,
    pagamento_id integer,
    valor numeric(10,2) DEFAULT 250.00 NOT NULL,
    descricao text,
    criado_em timestamp with time zone DEFAULT now() NOT NULL,
    consumido_em timestamp with time zone,
    consumido_lote_id integer,
    CONSTRAINT creditos_manutencao_tomador_check CHECK ((((entidade_id IS NOT NULL) AND (clinica_id IS NULL)) OR ((clinica_id IS NOT NULL) AND (entidade_id IS NULL))))
);


--
-- Name: creditos_manutencao_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.creditos_manutencao_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
--
-- Name: creditos_manutencao_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.creditos_manutencao_id_seq OWNED BY public.creditos_manutencao.id;
...
CREATE TABLE public.empresas_clientes (
    ...
    limite_primeira_cobranca_manutencao timestamp with time zone,
    manutencao_ja_cobrada boolean DEFAULT false NOT NULL
);
...
--
-- Name: COLUMN empresas_clientes.cartao_cnpj_path; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.empresas_clientes.cartao_cnpj_path IS 'Caminho do arquivo CartÃ£o CNPJ enviado no cadastro';


--
-- Name: COLUMN empresas_clientes.contrato_social_path; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN empresas_clientes.doc_identificacao_path; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.empresas_clientes.doc_identificacao_path IS 'Caminho do arquivo de identificaÃ§Ã£o do representante enviado no cadastro';


--
-- Name: COLUMN empresas_clientes.limite_primeira_cobranca_manutencao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.empresas_clientes.limite_primeira_cobranca_manutencao IS 'Data limite para emissão do 1º laudo para esta empresa (criado_em + 90 dias).';


--
-- Name: COLUMN empresas_clientes.manutencao_ja_cobrada; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.empresas_clientes.manutencao_ja_cobrada IS 'TRUE quando a taxa de manutenção de R$250 já foi gerada para esta empresa (evita duplicação).';
...
--
-- Name: entidade_configuracoes; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE entidade_configuracoes; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.entidade_configuracoes IS 'ConfiguraÃ§Ãµes de branding (logo, cores) por entidade';


--
-- Name: COLUMN entidade_configuracoes.logo_url; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.entidade_configuracoes.logo_url IS 'Logo em base64 data URI (data:image/...;base64,...) â€” max 256KB decoded';


--
-- Name: entidade_configuracoes_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: entidade_configuracoes_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: COLUMN entidades.limite_primeira_cobranca_manutencao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.entidades.limite_primeira_cobranca_manutencao IS 'Data limite para emissão do 1º laudo antes de cobrar taxa de manutenção (data_aceite + 90 dias).';


--
-- Name: COLUMN entidades.manutencao_ja_cobrada; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.entidades.manutencao_ja_cobrada IS 'TRUE quando a taxa de manutenção de R$250 já foi gerada para esta entidade (evita duplicação).';


--
-- Name: COLUMN entidades.credito_manutencao_pendente; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.entidades.credito_manutencao_pendente IS 'Crédito acumulado de taxas de manutenção pagas, para abater no próximo laudo.';
...
CREATE TABLE public.funcionarios (
    ...
    titulo_profissional character varying(100),
...
--
-- Name: COLUMN funcionarios_clinicas.setor; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.funcao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.matricula; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.nivel_cargo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.turno; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.escala; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.indice_avaliacao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_clinicas.data_ultimo_lote; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.setor; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.funcao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.matricula; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.nivel_cargo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.turno; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.escala; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.indice_avaliacao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN funcionarios_entidades.data_ultimo_lote; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE TABLE public.usuarios (
    ...
    reset_tentativas_falhas smallint DEFAULT 0 NOT NULL,
...
--
-- Name: COLUMN usuarios.reset_token; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.usuarios.reset_token IS 'Token de 64 chars hex gerado pelo admin para reset de senha';


--
-- Name: COLUMN usuarios.reset_token_expira_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.usuarios.reset_token_expira_em IS 'Expiração do token (7 dias após geração)';


--
-- Name: COLUMN usuarios.reset_tentativas_falhas; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.usuarios.reset_tentativas_falhas IS 'Tentativas falhas de uso do token (bloqueio após 3)';


--
-- Name: COLUMN usuarios.reset_usado_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.usuarios.reset_usado_em IS 'Data/hora em que o token foi usado (token inválido após isso)';


--
-- Name: COLUMN usuarios.asaas_wallet_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.usuarios.asaas_wallet_id IS 'ID da wallet Asaas do usuÃ¡rio para recebimento de splits de comissÃ£o (admin, comercial).';
...
--
-- Name: hierarquia_comercial; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: COLUMN hierarquia_comercial.data_fim; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.hierarquia_comercial.data_fim IS 'Data em que o vÃ­nculo foi encerrado (inativaÃ§Ã£o). NULL = vÃ­nculo ativo.';


--
-- Name: hierarquia_comercial_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: importacoes_clinica; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.importacoes_clinica (
    ...
    status character varying(50) DEFAULT 'concluido'::character varying NOT NULL,
...
--
-- Name: importacoes_clinica_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: importacoes_clinica_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
CREATE TABLE public.laudos (
    ...
    zapsign_doc_token character varying(255),
    zapsign_signer_token character varying(255),
    zapsign_status character varying(50) DEFAULT 'pending'::character varying,
    assinado_em timestamp without time zone,
    pdf_gerado_em timestamp without time zone,
    ...
    CONSTRAINT chk_laudos_status_valid CHECK (((status)::text = ANY ((ARRAY['rascunho'::character varying, 'pdf_gerado'::character varying, 'aguardando_assinatura'::character varying, 'assinado_processando'::character varying, 'emitido'::character varying, 'enviado'::character varying])::text[]))),
...
--
-- Name: COLUMN laudos.zapsign_doc_token; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos.zapsign_signer_token; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos.zapsign_status; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos.assinado_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos.pdf_gerado_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos.zapsign_sign_url; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.laudos.zapsign_sign_url IS 'URL direta de assinatura retornada pelo ZapSign. Usada como fallback caso o email nÃ£o chegue ao assinante.';
...
--
-- Name: laudos_storage_log; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.laudos_storage_log (
    ...
    backblaze_bucket character varying(255),
    backblaze_key character varying(1024),
    backblaze_url text,
    emissor_cpf character(11),
...
--
-- Name: TABLE laudos_storage_log; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos_storage_log.arquivo_path; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN laudos_storage_log.hash_sha256; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: leads_representante; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.leads_representante (
    ...
    percentual_comissao_comercial numeric(5,2) DEFAULT 0 NOT NULL,
    valor_custo_fixo_snapshot numeric(10,2) DEFAULT NULL::numeric,
    requer_aprovacao_suporte boolean DEFAULT false NOT NULL,
    ...
    CONSTRAINT chk_leads_perc_total_max CHECK (((COALESCE(percentual_comissao_representante, (0)::numeric) + COALESCE(percentual_comissao_comercial, (0)::numeric)) <= (40)::numeric)),
    CONSTRAINT lead_cnpj_valido CHECK ((cnpj ~ '^\d{14}$'::text)),
    CONSTRAINT lead_valor_negociado_positivo CHECK ((valor_negociado >= (0)::numeric)),
    CONSTRAINT leads_perc_comercial_range CHECK (((percentual_comissao_comercial >= (0)::numeric) AND (percentual_comissao_comercial <= (40)::numeric))),
...
--
-- Name: TABLE leads_representante; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN leads_representante.data_expiracao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN leads_representante.entidade_id; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN leads_representante.token_atual; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN leads_representante.valor_negociado; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.leads_representante.valor_negociado IS 'Valor negociado pelo representante com a empresa no momento da indicaÃ§Ã£o. ObrigatÃ³rio para novos leads.';


--
-- Name: COLUMN leads_representante.percentual_comissao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.leads_representante.percentual_comissao IS 'Percentual de comissão do representante para este lead (0-40%). Alias de percentual_comissao_representante.';


--
-- Name: COLUMN leads_representante.percentual_comissao_representante; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.leads_representante.percentual_comissao_representante IS 'Percentual de comissÃ£o do representante para este lead (0-40%)';


--
-- Name: COLUMN leads_representante.percentual_comissao_comercial; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.leads_representante.percentual_comissao_comercial IS 'Percentual de comissão do comercial gravado no momento do cadastro do lead.';


--
-- Name: COLUMN leads_representante.valor_custo_fixo_snapshot; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.leads_representante.valor_custo_fixo_snapshot IS 'Snapshot do valor_custo_fixo do representante no momento do cadastro do lead (modelo custo_fixo). NULL = modelo percentual.';


--
-- Name: COLUMN leads_representante.requer_aprovacao_suporte; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.leads_representante.requer_aprovacao_suporte IS 'true quando o valor QWork fica abaixo do custo mínimo por avaliação (R$5 clínica / R$12 entidade). Requer aprovação do suporte além do comercial.';


--
-- Name: leads_representante_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: leads_representante_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.status_pagamento; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.solicitacao_emissao_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.valor_por_funcionario; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.link_pagamento_token; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.link_pagamento_expira_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.link_pagamento_enviado_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.pagamento_metodo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.pagamento_parcelas; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.pago_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN lotes_avaliacao.valor_servico; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE TABLE public.notificacoes (
    ...
    CONSTRAINT notificacoes_destinatario_tipo_check CHECK ((destinatario_tipo = ANY (ARRAY['admin'::text, 'gestor'::text, 'funcionario'::text, 'contratante'::text, 'clinica'::text, 'comercial'::text, 'suporte'::text])))
);
...
CREATE TABLE public.pagamentos (
    ...
    asaas_due_date date,
    asaas_payment_id character varying(50),
    tipo_cobranca character varying(20) DEFAULT 'laudo'::character varying NOT NULL,
    empresa_id integer,
    link_pagamento_token character varying(255),
    link_pagamento_enviado_em timestamp with time zone,
    link_disponibilizado_em timestamp with time zone,
    ...
    CONSTRAINT pagamentos_tipo_cobranca_check CHECK (((tipo_cobranca)::text = ANY ((ARRAY['laudo'::character varying, 'manutencao'::character varying])::text[])))
);
...
--
-- Name: COLUMN pagamentos.asaas_customer_id; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN pagamentos.asaas_payment_url; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.pagamentos.asaas_payment_url IS 'URL de checkout Asaas (para cartÆo)';


--
-- Name: COLUMN pagamentos.asaas_boleto_url; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.pagamentos.asaas_boleto_url IS 'URL do boleto banc rio';


--
-- Name: COLUMN pagamentos.asaas_invoice_url; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN pagamentos.asaas_pix_qrcode; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.pagamentos.asaas_pix_qrcode IS 'C¢digo PIX Copia e Cola';


--
-- Name: COLUMN pagamentos.asaas_pix_qrcode_image; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN pagamentos.asaas_net_value; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.pagamentos.asaas_net_value IS 'Valor l¡quido ap¢s dedu‡Æo de taxas Asaas';


--
-- Name: COLUMN pagamentos.asaas_due_date; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN pagamentos.asaas_payment_id; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN pagamentos.tipo_cobranca; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.pagamentos.tipo_cobranca IS 'Tipo da cobrança: laudo (emissão) ou manutencao (taxa de manutenção R$250).';


--
-- Name: COLUMN pagamentos.empresa_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.pagamentos.empresa_id IS 'ID da empresa_cliente quando tipo_cobranca=manutencao para clínicas.';
...
ALTER TABLE ONLY public.questao_condicoes FORCE ROW LEVEL SECURITY;
...
COMMENT ON TABLE public.rate_limit_entries IS 'Entradas de rate limiting distribuÃ­do. Chave pode ser IP (rate-limit:ip:<ip>) ou usuÃ¡rio (rate-limit:user:<hash>). Registros expirados sÃ£o deletados periodicamente pelo aplicativo.';


--
-- Name: COLUMN rate_limit_entries.key; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.rate_limit_entries.key IS 'Identificador Ãºnico da janela de rate. Ex: rate-limit:ip:1.2.3.4 ou rate-limit:user:abc123';


--
-- Name: COLUMN rate_limit_entries.count; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.rate_limit_entries.count IS 'NÃºmero de requisiÃ§Ãµes na janela atual';
...
ALTER TABLE ONLY public.relatorio_templates FORCE ROW LEVEL SECURITY;
...
CREATE TABLE public.representantes (
    ...
    reset_tentativas_falhas smallint DEFAULT 0 NOT NULL,
    reset_usado_em timestamp without time zone,
    modelo_comissionamento public.modelo_comissionamento,
    asaas_wallet_id character varying(50),
    percentual_comissao_comercial numeric(5,2) DEFAULT 0 NOT NULL,
    valor_custo_fixo_entidade numeric(10,2) DEFAULT NULL::numeric,
    valor_custo_fixo_clinica numeric(10,2) DEFAULT NULL::numeric,
    ativo boolean DEFAULT true NOT NULL,
    gestor_comercial_cpf character varying(11),
    CONSTRAINT rep_custo_fixo_clinica_pos CHECK (((valor_custo_fixo_clinica IS NULL) OR (valor_custo_fixo_clinica > (0)::numeric))),
    CONSTRAINT rep_custo_fixo_entidade_pos CHECK (((valor_custo_fixo_entidade IS NULL) OR (valor_custo_fixo_entidade > (0)::numeric))),
    ...
    CONSTRAINT representantes_perc_comercial_range CHECK (((percentual_comissao_comercial >= (0)::numeric) AND (percentual_comissao_comercial <= (40)::numeric)))
);


--
-- Name: TABLE representantes; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.tipo_pessoa; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.codigo; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.status; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.status IS 'Status do representante: apto, aguardando_senha, expirado, desativado, rejeitado';


--
-- Name: COLUMN representantes.percentual_comissao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.percentual_comissao IS 'DEPRECADO: comissÃ£o agora Ã© por lead/vÃ­nculo (vinculos_comissao.percentual_comissao_representante). Mantido para fallback em legado.';


--
-- Name: COLUMN representantes.senha_hash; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.senha_repres; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.convite_token; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.convite_expira_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.convite_tentativas_falhas; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.convite_usado_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.aceite_politica_privacidade; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.aceite_politica_privacidade_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes.reset_token; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.reset_token IS 'Token de 64 chars hex para reset de senha via admin';


--
-- Name: COLUMN representantes.reset_token_expira_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.reset_token_expira_em IS 'Expiração do token (7 dias)';


--
-- Name: COLUMN representantes.reset_tentativas_falhas; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.reset_tentativas_falhas IS 'Tentativas falhas (bloqueio após 3)';


--
-- Name: COLUMN representantes.reset_usado_em; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.reset_usado_em IS 'Data de uso do token';


--
-- Name: COLUMN representantes.modelo_comissionamento; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.modelo_comissionamento IS 'Modelo de comissÃ£o definido pelo Comercial no EstÃ¡gio 1 de aprovaÃ§Ã£o. NULL = ainda nÃ£o aprovado pelo comercial.';


--
-- Name: COLUMN representantes.asaas_wallet_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.asaas_wallet_id IS 'walletId da subconta Asaas criada pelo Suporte no EstÃ¡gio 2 de aprovaÃ§Ã£o. NULL = subconta ainda nÃ£o criada.';


--
-- Name: COLUMN representantes.percentual_comissao_comercial; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.percentual_comissao_comercial IS 'Percentual de comissão reservado ao comercial para este representante. A soma com percentual_comissao não pode ultrapassar 40%.';


--
-- Name: COLUMN representantes.valor_custo_fixo_entidade; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.valor_custo_fixo_entidade IS 'Valor fixo retido pela QWork por avaliação no modelo custo_fixo (tipo entidade). Quando NULL, usa CUSTO_POR_AVALIACAO.entidade = 12.';


--
-- Name: COLUMN representantes.valor_custo_fixo_clinica; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.valor_custo_fixo_clinica IS 'Valor fixo retido pela QWork por avaliação no modelo custo_fixo (tipo clínica). Quando NULL, usa CUSTO_POR_AVALIACAO.clinica = 5.';


--
-- Name: COLUMN representantes.ativo; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.ativo IS 'Controla acesso ao sistema. false = login bloqueado. Independente do status de negÃ³cio.';


--
-- Name: COLUMN representantes.gestor_comercial_cpf; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes.gestor_comercial_cpf IS 'CPF do usuÃ¡rio comercial responsÃ¡vel por este representante. NULL = visÃ­vel apenas para admin/suporte.';


--
-- Name: representantes_cadastro_leads; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.representantes_cadastro_leads (
    ...
    representante_id integer,
    asaas_wallet_id character varying(200),
...
--
-- Name: TABLE representantes_cadastro_leads; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes_cadastro_leads.doc_cpf_key; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes_cadastro_leads.status; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN representantes_cadastro_leads.representante_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.representantes_cadastro_leads.representante_id IS 'FK para representantes: preenchido quando o lead Ã© convertido em representante oficial';
...
--
-- Name: representantes_senhas; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE representantes_senhas; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.representantes_senhas IS 'Senhas de representantes â€” substitui senha_repres inline. PadrÃ£o anÃ¡logo a entidades_senhas/clinicas_senhas.';


--
-- Name: representantes_senhas_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: representantes_senhas_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: seq_representante_codigo; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: SEQUENCE seq_representante_codigo; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON SEQUENCE public.seq_representante_codigo IS 'SequÃªncia para geraÃ§Ã£o de cÃ³digos numÃ©ricos de representantes (a partir de 100)';


--
-- Name: seq_vendedor_codigo; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: SEQUENCE seq_vendedor_codigo; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON SEQUENCE public.seq_vendedor_codigo IS 'SequÃªncia para geraÃ§Ã£o de cÃ³digos numÃ©ricos de vendedores (a partir de 100)';
...
ALTER TABLE ONLY public.templates_contrato FORCE ROW LEVEL SECURITY;
...
--
-- Name: v_solicitacoes_emissao; Type: VIEW; Schema: public; Owner: -
--
...
SELECT
    ...
    NULL::numeric(5,2) AS representante_percentual_comissao_comercial,
    NULL::public.modelo_comissionamento AS modelo_comissionamento,
    ...
    NULL::numeric(10,2) AS valor_custo_fixo_snapshot,
    NULL::numeric(12,2) AS valor_negociado_vinculo;


--
-- Name: VIEW v_solicitacoes_emissao; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON VIEW public.v_solicitacoes_emissao IS 'View para admin gerenciar solicitaÃ§Ãµes de emissÃ£o. valor_total_calculado usa COALESCE(valor_por_funcionario, lead_valor_negociado, valor_negociado_vinculo) para mostrar estimativa mesmo antes do admin definir o valor. comissoes_geradas_count: total de comissÃµes criadas para o lote (inclui provisionadas futuras). comissoes_ativas_count: comissÃµes com parcela_confirmada_em IS NOT NULL (parcela efetivamente paga).';


--
-- Name: vendedores_dados_bancarios; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE vendedores_dados_bancarios; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.vendedores_dados_bancarios IS 'Dados bancÃ¡rios de vendedores â€” acesso restrito ao perfil suporte. EdiÃ§Ãµes sÃ£o auditadas em comissionamento_auditoria.';


--
-- Name: vendedores_dados_bancarios_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: vendedores_dados_bancarios_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vendedores_perfil.convite_token; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vendedores_perfil.convite_token IS 'Token de uso Ãºnico para criaÃ§Ã£o de senha (hex-64, expira em 7 dias)';


--
-- Name: COLUMN vendedores_perfil.primeira_senha_alterada; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vendedores_perfil.primeira_senha_alterada IS 'Flag: vendedor trocou a senha provisÃ³ria no primeiro acesso';


--
-- Name: COLUMN vendedores_perfil.aceite_termos; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vendedores_perfil.aceite_politica_privacidade; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vendedores_perfil.aceite_politica_privacidade IS 'Vendedor aceitou PolÃ­tica de Privacidade';


--
-- Name: COLUMN vendedores_perfil.aceite_disclaimer_nv; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vendedores_perfil.aceite_disclaimer_nv IS 'Vendedor aceitou Contrato de RepresentaÃ§Ã£o NÃƒO-CLT';


--
-- Name: vendedores_perfil_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.vinculos_comissao (
    ...
    CONSTRAINT vinculos_perc_comercial_range CHECK (((percentual_comissao_comercial >= (0)::numeric) AND (percentual_comissao_comercial <= (40)::numeric)))
);


--
-- Name: TABLE vinculos_comissao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vinculos_comissao.representante_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vinculos_comissao.representante_id IS 'FK para representantes. NULL quando o cliente foi cadastrado sem representante vinculado. O Comercial pode atribuir retroativamente.';


--
-- Name: COLUMN vinculos_comissao.data_expiracao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vinculos_comissao.ultimo_laudo_em; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vinculos_comissao.clinica_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vinculos_comissao.clinica_id IS 'FK para clinicas: preenchido quando o tomador Ã© uma clÃ­nica cadastrada via fluxo RH (sem entidade espelho). Mutuamente exclusivo com entidade_id.';


--
-- Name: COLUMN vinculos_comissao.valor_negociado; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vinculos_comissao.percentual_comissao_representante; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN vinculos_comissao.percentual_comissao_comercial; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.vinculos_comissao.percentual_comissao_comercial IS 'Percentual de comissão do comercial copiado do representante no momento da criação do vínculo.';


--
-- Name: vinculos_comissao_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--
...
--
-- Name: vw_auditoria_acessos_rh; Type: VIEW; Schema: public; Owner: -
--
...
--
-- Name: VIEW vw_auditoria_acessos_rh; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: vw_auditoria_avaliacoes; Type: VIEW; Schema: public; Owner: -
--
...
--
-- Name: VIEW vw_auditoria_avaliacoes; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN webhook_logs.event; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN webhook_logs.processing_duration_ms; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_entidade id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_usuario id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: audit_delecoes_tomador id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.audit_delecoes_tomador ALTER COLUMN id SET DEFAULT nextval('public.audit_delecoes_tomador_id_seq'::regclass);
...
--
-- Name: comissionamento_auditoria id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: creditos_manutencao id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao ALTER COLUMN id SET DEFAULT nextval('public.creditos_manutencao_id_seq'::regclass);
...
--
-- Name: entidade_configuracoes id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: importacoes_clinica id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: leads_representante id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: representantes_senhas id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_dados_bancarios id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao id; Type: DEFAULT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_entidade aceites_termos_entidade_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_usuario aceites_termos_usuario_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: audit_delecoes_tomador audit_delecoes_tomador_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.audit_delecoes_tomador
    ADD CONSTRAINT audit_delecoes_tomador_pkey PRIMARY KEY (id);
...
--
-- Name: comissionamento_auditoria comissionamento_auditoria_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo comissoes_laudo_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade confirmacao_identidade_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: creditos_manutencao creditos_manutencao_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao
    ADD CONSTRAINT creditos_manutencao_pkey PRIMARY KEY (id);
...
--
-- Name: entidade_configuracoes entidade_configuracoes_entidade_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: entidade_configuracoes entidade_configuracoes_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hierarquia_comercial_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hierarquia_comercial_vendedor_rep_unico; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: importacoes_clinica importacoes_clinica_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log laudos_storage_log_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: leads_representante leads_representante_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: leads_representante leads_representante_token_atual_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: lotes_avaliacao link_pagamento_token_unique; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes_cadastro_leads representantes_cadastro_leads_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes representantes_cnpj_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes representantes_codigo_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes representantes_convite_token_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes representantes_cpf_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes representantes_email_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes_senhas representantes_senhas_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes representantes_somente_pj; Type: CHECK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE public.representantes
    ADD CONSTRAINT representantes_somente_pj CHECK ((tipo_pessoa = 'pj'::public.tipo_pessoa_representante)) NOT VALID;
...
--
-- Name: aceites_termos_entidade uk_aceite_entidade; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: aceites_termos_usuario uk_aceite_usuario; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: webhook_logs uq_webhook_logs_payment_event; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_dados_bancarios vendedores_dados_bancarios_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_dados_bancarios vendedores_dados_bancarios_usuario_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil vendedores_perfil_cnpj_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil vendedores_perfil_codigo_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil vendedores_perfil_convite_token_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil vendedores_perfil_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil vendedores_perfil_usuario_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao vinculos_comissao_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: idx_aceites_entidade_cnpj; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_aceites_entidade_data; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_aceites_entidade_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_aceites_usuario_cpf; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_aceites_usuario_data; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_aceites_usuario_termo; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_audit_delecoes_tomador_cnpj; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_audit_delecoes_tomador_cnpj ON public.audit_delecoes_tomador USING btree (cnpj);


--
-- Name: idx_audit_delecoes_tomador_criado_em; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_audit_delecoes_tomador_criado_em ON public.audit_delecoes_tomador USING btree (criado_em DESC);
...
--
-- Name: idx_avaliacoes_identificacao_confirmada; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_cadastro_leads_cnpj; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_cadastro_leads_cpf; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_cadastro_leads_criado_em; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_cadastro_leads_email; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_cadastro_leads_status; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_clinicas_entidade_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissionamento_auditoria_criado_em; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissionamento_auditoria_tabela_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_clinica_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_laudo_asaas_payment; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comissoes_laudo_asaas_payment ON public.comissoes_laudo USING btree (asaas_payment_id) WHERE (asaas_payment_id IS NOT NULL);


--
-- Name: idx_comissoes_laudo_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_laudo_lote_parcela; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_mes_pagamento; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_parcela_pendente; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_representante_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_status; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_comissoes_vinculo_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_confirmacao_avaliacao_id; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_confirmacao_avaliacao_id ON public.confirmacao_identidade USING btree (avaliacao_id);


--
-- Name: idx_confirmacao_data; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_confirmacao_data ON public.confirmacao_identidade USING btree (confirmado_em);


--
-- Name: idx_confirmacao_funcionario_cpf; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_creditos_manutencao_clinica; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_creditos_manutencao_clinica ON public.creditos_manutencao USING btree (clinica_id) WHERE (clinica_id IS NOT NULL);


--
-- Name: idx_creditos_manutencao_entidade; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_creditos_manutencao_entidade ON public.creditos_manutencao USING btree (entidade_id) WHERE (entidade_id IS NOT NULL);
...
--
-- Name: idx_empresas_limite_manutencao; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_empresas_limite_manutencao ON public.empresas_clientes USING btree (limite_primeira_cobranca_manutencao) WHERE ((limite_primeira_cobranca_manutencao IS NOT NULL) AND (manutencao_ja_cobrada = false));


--
-- Name: idx_entidade_configuracoes_entidade_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_entidades_limite_manutencao; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_entidades_limite_manutencao ON public.entidades USING btree (limite_primeira_cobranca_manutencao) WHERE ((limite_primeira_cobranca_manutencao IS NOT NULL) AND (manutencao_ja_cobrada = false));
...
--
-- Name: idx_func_clinicas_nivel_cargo; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_func_entidades_nivel_cargo; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_hierarquia_comercial_comercial_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_hierarquia_comercial_representante_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_hierarquia_comercial_vendedor_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_importacoes_clinica_clinica_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_importacoes_clinica_criado_em; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_laudos_storage_log_laudo_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_laudos_storage_log_registrado_em; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_laudos_zapsign_doc_token; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_laudos_zapsign_doc_token ON public.laudos USING btree (zapsign_doc_token) WHERE (zapsign_doc_token IS NOT NULL);


--
-- Name: idx_leads_cnpj; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_leads_data_expiracao; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_leads_entidade_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_leads_representante_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_leads_representante_vendedor_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_lotes_avaliacao_expiracao_pagamento; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_lotes_avaliacao_solicitacao_emissao; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_lotes_avaliacao_status_pagamento; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_lotes_avaliacao_token_pagamento; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_lotes_entidade_clinica; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_lotes_entidade_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_pagamentos_asaas_customer_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_pagamentos_asaas_payment_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_pagamentos_link_token; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_pagamentos_link_token ON public.pagamentos USING btree (link_pagamento_token) WHERE (link_pagamento_token IS NOT NULL);
...
--
-- Name: idx_pagamentos_plataforma_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_pagamentos_tipo_cobranca; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pagamentos_tipo_cobranca ON public.pagamentos USING btree (tipo_cobranca);
...
--
-- Name: idx_rate_limit_expires_at; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_rate_limit_expires_at ON public.rate_limit_entries USING btree (expires_at);
...
--
-- Name: idx_representantes_ativo; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_representantes_ativo ON public.representantes USING btree (ativo);


--
-- Name: idx_representantes_cnpj; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_codigo; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_convite_token; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_cpf; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_dados_bancarios_status; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_gestor_comercial_cpf; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_representantes_gestor_comercial_cpf ON public.representantes USING btree (gestor_comercial_cpf);


--
-- Name: idx_representantes_reset_token; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_representantes_reset_token ON public.representantes USING btree (reset_token) WHERE (reset_token IS NOT NULL);


--
-- Name: idx_representantes_senhas_rep_cpf; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_status; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_representantes_status_ativo; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_representantes_status_ativo ON public.representantes USING btree (status, ativo);
...
--
-- Name: idx_usuarios_reset_token; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_usuarios_reset_token ON public.usuarios USING btree (reset_token) WHERE (reset_token IS NOT NULL);
...
--
-- Name: idx_vendedores_dados_bancarios_usuario; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vendedores_perfil_cnpj; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vendedores_perfil_codigo; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vendedores_perfil_convite_token; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vendedores_perfil_tipo_pessoa; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vinculos_data_expiracao; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vinculos_entidade_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vinculos_representante_id; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_vinculos_status; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_webhook_logs_errors; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_webhook_logs_processed_at; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: leads_cnpj_ativo_unique; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: vinculo_unico_clinica; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: vinculo_unico_entidade; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: v_solicitacoes_emissao _RETURN; Type: RULE; Schema: public; Owner: -
--
...
--
-- Name: laudos trg_audit_laudo_delete_attempt; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: laudos trg_bloquear_campos_emissor; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo trg_comissao_status_audit; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo trg_comissoes_atualizado_em; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial trg_hierarquia_comercial_updated_at; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: leads_representante trg_leads_atualizado_em; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log trg_prevent_mutation_storage_log; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: avaliacoes trg_recalc_lote_on_avaliacao_change; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_recalc_lote_on_avaliacao_change AFTER INSERT OR DELETE ON public.avaliacoes FOR EACH ROW EXECUTE FUNCTION public.fn_recalcular_status_lote_on_avaliacao_update();
...
--
-- Name: TRIGGER trg_reject_prohibited_roles ON funcionarios; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TRIGGER trg_reject_prohibited_roles ON public.funcionarios IS 'Migration 410: BEFORE INSERT/UPDATE â€” rejeita usuario_tipo de conta sistema em funcionarios.';


--
-- Name: representantes trg_representante_codigo; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: representantes trg_representante_status_audit; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: representantes trg_representantes_atualizado_em; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: representantes_senhas trg_representantes_senhas_atualizado; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: vendedores_dados_bancarios trg_vendedores_dados_bancarios_atualizado; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil trg_vendedores_perfil_atualizado; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao trg_vinculo_status_audit; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao trg_vinculos_atualizado_em; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: avaliacoes trigger_atualizar_ultima_avaliacao; Type: TRIGGER; Schema: public; Owner: -
--
...
--
-- Name: clinicas clinicas_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.clinicas
    ADD CONSTRAINT clinicas_entidade_id_fkey FOREIGN KEY (entidade_id) REFERENCES public.entidades(id) ON DELETE SET NULL;


--
-- Name: comissoes_laudo comissoes_laudo_clinica_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo comissoes_laudo_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo comissoes_laudo_laudo_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo comissoes_laudo_lote_pagamento_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo comissoes_laudo_representante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: comissoes_laudo comissoes_laudo_vinculo_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade confirmacao_identidade_avaliacao_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade confirmacao_identidade_funcionario_cpf_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.confirmacao_identidade
    ADD CONSTRAINT confirmacao_identidade_funcionario_cpf_fkey FOREIGN KEY (funcionario_cpf) REFERENCES public.funcionarios(cpf) ON DELETE CASCADE;


--
-- Name: creditos_manutencao creditos_manutencao_clinica_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao
    ADD CONSTRAINT creditos_manutencao_clinica_id_fkey FOREIGN KEY (clinica_id) REFERENCES public.clinicas(id) ON DELETE CASCADE;


--
-- Name: creditos_manutencao creditos_manutencao_consumido_lote_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao
    ADD CONSTRAINT creditos_manutencao_consumido_lote_id_fkey FOREIGN KEY (consumido_lote_id) REFERENCES public.lotes_avaliacao(id) ON DELETE SET NULL;


--
-- Name: creditos_manutencao creditos_manutencao_empresa_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao
    ADD CONSTRAINT creditos_manutencao_empresa_id_fkey FOREIGN KEY (empresa_id) REFERENCES public.empresas_clientes(id) ON DELETE SET NULL;


--
-- Name: creditos_manutencao creditos_manutencao_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao
    ADD CONSTRAINT creditos_manutencao_entidade_id_fkey FOREIGN KEY (entidade_id) REFERENCES public.entidades(id) ON DELETE CASCADE;


--
-- Name: creditos_manutencao creditos_manutencao_pagamento_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.creditos_manutencao
    ADD CONSTRAINT creditos_manutencao_pagamento_id_fkey FOREIGN KEY (pagamento_id) REFERENCES public.pagamentos(id) ON DELETE SET NULL;
...
--
-- Name: entidade_configuracoes entidade_configuracoes_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: entidades_senhas entidades_senhas_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.entidades_senhas
    ADD CONSTRAINT entidades_senhas_entidade_id_fkey FOREIGN KEY (entidade_id) REFERENCES public.entidades(id) ON DELETE CASCADE;
...
--
-- Name: confirmacao_identidade fk_avaliacao; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.confirmacao_identidade
    ADD CONSTRAINT fk_avaliacao FOREIGN KEY (avaliacao_id) REFERENCES public.avaliacoes(id) ON DELETE CASCADE;
...
--
-- Name: confirmacao_identidade fk_funcionario; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.confirmacao_identidade
    ADD CONSTRAINT fk_funcionario FOREIGN KEY (funcionario_cpf) REFERENCES public.funcionarios(cpf) ON DELETE CASCADE;
...
--
-- Name: laudos_storage_log fk_laudos_storage_log_laudo; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.laudos_storage_log
    ADD CONSTRAINT fk_laudos_storage_log_laudo FOREIGN KEY (laudo_id) REFERENCES public.laudos(id) ON DELETE RESTRICT;
...
--
-- Name: hierarquia_comercial hierarquia_comercial_comercial_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hierarquia_comercial_representante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hierarquia_comercial_vendedor_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: importacoes_clinica importacoes_clinica_clinica_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.importacoes_clinica
    ADD CONSTRAINT importacoes_clinica_clinica_id_fkey FOREIGN KEY (clinica_id) REFERENCES public.clinicas(id);
...
--
-- Name: leads_representante leads_representante_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: leads_representante leads_representante_representante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: leads_representante leads_representante_vendedor_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: lotes_avaliacao lotes_avaliacao_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
ALTER TABLE ONLY public.pagamentos
    ADD CONSTRAINT pagamentos_empresa_id_fkey FOREIGN KEY (empresa_id) REFERENCES public.empresas_clientes(id) ON DELETE SET NULL;
...
--
-- Name: representantes representantes_bloqueio_conflito_pf_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes_cadastro_leads representantes_cadastro_leads_representante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes_senhas representantes_senhas_representante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_dados_bancarios vendedores_dados_bancarios_usuario_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vendedores_perfil vendedores_perfil_usuario_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao vinculos_comissao_clinica_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao vinculos_comissao_entidade_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao vinculos_comissao_lead_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: vinculos_comissao vinculos_comissao_representante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: representantes_cadastro_leads admin_cadastro_leads_all; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: comissionamento_auditoria auditoria_admin_only; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: avaliacoes avaliacoes_gestor_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: POLICY avaliacoes_gestor_select ON avaliacoes; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: avaliacoes avaliacoes_rh_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: POLICY avaliacoes_rh_select ON avaliacoes; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: clinica_configuracoes clinica_config_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY clinica_config_modify ON public.clinica_configuracoes USING ((((clinica_id)::text = current_setting('app.current_user_clinica_id'::text, true)) OR (current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text))) WITH CHECK ((((clinica_id)::text = current_setting('app.current_user_clinica_id'::text, true)) OR (current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: clinica_configuracoes clinica_config_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY clinica_config_select ON public.clinica_configuracoes FOR SELECT USING ((((clinica_id)::text = current_setting('app.current_user_clinica_id'::text, true)) OR (current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text, 'emissor'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: clinica_configuracoes; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.clinica_configuracoes ENABLE ROW LEVEL SECURITY;
...
--
-- Name: comissionamento_auditoria; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.comissionamento_auditoria ENABLE ROW LEVEL SECURITY;

--
-- Name: comissoes_laudo; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.comissoes_laudo ENABLE ROW LEVEL SECURITY;

--
-- Name: comissoes_laudo comissoes_rep_own; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hc_admin_comercial_all; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hc_suporte_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial hc_vendedor_own; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: hierarquia_comercial; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: laudos laudos_gestor_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: POLICY laudos_gestor_select ON laudos; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: laudos laudos_no_delete_app_roles; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: POLICY laudos_no_delete_app_roles ON laudos; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: laudos laudos_rh_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: POLICY laudos_rh_select ON laudos; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.laudos_storage_log ENABLE ROW LEVEL SECURITY;

--
-- Name: laudos_storage_log laudos_storage_log_insert_only; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: laudos_storage_log laudos_storage_log_select_authorized; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: leads_representante leads_rep_own; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: leads_representante; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: lotes_avaliacao lotes_gestor_entidade_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY lotes_gestor_entidade_select ON public.lotes_avaliacao FOR SELECT USING (((current_setting('app.current_user_perfil'::text, true) = 'gestor'::text) AND (entidade_id IS NOT NULL) AND (entidade_id = (NULLIF(current_setting('app.current_user_entidade_id'::text, true), ''::text))::integer)));


--
-- Name: lotes_avaliacao lotes_gestor_entidade_update; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY lotes_gestor_entidade_update ON public.lotes_avaliacao FOR UPDATE USING (((current_setting('app.current_user_perfil'::text, true) = 'gestor'::text) AND (entidade_id IS NOT NULL) AND (entidade_id = (NULLIF(current_setting('app.current_user_entidade_id'::text, true), ''::text))::integer)));


--
-- Name: lotes_avaliacao lotes_gestor_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: POLICY lotes_gestor_select ON lotes_avaliacao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: POLICY policy_lotes_admin ON lotes_avaliacao; Type: COMMENT; Schema: public; Owner: -
--
...
CREATE POLICY policy_lotes_emissor ON public.lotes_avaliacao FOR SELECT USING (((public.current_user_perfil() = 'emissor'::text) AND ((status)::text = ANY ((ARRAY['pendente'::character varying, 'em_processamento'::character varying, 'concluido'::character varying])::text[]))));


--
-- Name: POLICY policy_lotes_emissor ON lotes_avaliacao; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: questao_condicoes; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.questao_condicoes ENABLE ROW LEVEL SECURITY;

--
-- Name: questao_condicoes questao_condicoes_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY questao_condicoes_modify ON public.questao_condicoes USING (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text))) WITH CHECK (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: questao_condicoes questao_condicoes_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY questao_condicoes_select ON public.questao_condicoes FOR SELECT USING (((current_setting('app.current_perfil'::text, true) IS NOT NULL) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: relatorio_templates; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.relatorio_templates ENABLE ROW LEVEL SECURITY;

--
-- Name: relatorio_templates relatorio_templates_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY relatorio_templates_modify ON public.relatorio_templates USING (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text))) WITH CHECK (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: relatorio_templates relatorio_templates_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY relatorio_templates_select ON public.relatorio_templates FOR SELECT USING (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text, 'emissor'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: representantes rep_insert_public; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: representantes rep_sees_own; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: representantes rep_update_own; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: representantes_cadastro_leads; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: laudos rls_emissor_insert_laudos; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: laudos rls_emissor_select_laudos; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: laudos rls_emissor_update_laudos; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: confirmacao_identidade system_insert_confirmations; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: templates_contrato; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.templates_contrato ENABLE ROW LEVEL SECURITY;

--
-- Name: templates_contrato templates_contrato_modify; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY templates_contrato_modify ON public.templates_contrato USING (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text))) WITH CHECK (((current_setting('app.current_perfil'::text, true) = ANY (ARRAY['admin'::text, 'suporte'::text])) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: templates_contrato templates_contrato_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY templates_contrato_select ON public.templates_contrato FOR SELECT USING (((current_setting('app.current_perfil'::text, true) IS NOT NULL) OR (current_setting('app.is_backend'::text, true) = '1'::text)));


--
-- Name: vinculos_comissao; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.vinculos_comissao ENABLE ROW LEVEL SECURITY;

--
-- Name: vinculos_comissao vinculos_rep_own; Type: POLICY; Schema: public; Owner: -
--
