...
--
-- Name: empresas_clientes admin_all_empresas; Type: POLICY; Schema: public; Owner: -
--
...
CREATE POLICY admin_restricted_funcionarios ON public.funcionarios USING (((current_setting('app.current_user_perfil'::text, true) = 'admin'::text) AND ((perfil)::text = ANY ((ARRAY['rh'::character varying, 'emissor'::character varying])::text[]))));
...
CREATE POLICY lotes_rh_update ON public.lotes_avaliacao FOR UPDATE USING (((public.current_user_perfil() = 'rh'::text) AND (clinica_id = public.current_user_clinica_id()) AND ((status)::text <> ALL ((ARRAY['finalizado'::character varying, 'cancelado'::character varying])::text[]))));
