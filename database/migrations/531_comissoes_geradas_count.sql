-- ============================================================================
-- Migration 531: Adicionar comissoes_geradas_count à v_solicitacoes_emissao
--
-- OBJETIVO: Expor no frontend quantas comissões já foram geradas para cada lote.
--   - comissao_gerada (bool)  : mantido para compatibilidade — EXISTS(...)
--   - comissoes_geradas_count : novo — COUNT(*) das comissões do lote
--
-- Com parcelamento (ex: 5x), um lote pode ter até 5 registros em comissoes_laudo
-- (um por parcela paga). O frontend usa comissoes_geradas_count para exibir
-- "2/5 parcelas com comissão" e decidir se o botão "Gerar Comissão" está habilitado.
--
-- Data: 2026-03-15
-- ============================================================================
...
SELECT
  ...
  -- Contagem de comissões geradas (uma por parcela paga)
  (
...
COMMENT ON VIEW v_solicitacoes_emissao IS
  ...
  'comissoes_geradas_count: total de comissões geradas para o lote (uma por parcela paga).';
