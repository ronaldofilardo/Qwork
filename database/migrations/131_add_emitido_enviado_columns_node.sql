...
--
-- VERSÃO SEM \echo (para execução via Node.js)
-- ==========================================
...
BEGIN
    IF NOT EXISTS (
        ...
        -- Verificar se tabela setores existe antes de adicionar FK
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'setores') THEN
            ...
            RAISE NOTICE 'Coluna setor_id adicionada com FK';
        ELSE
            ALTER TABLE lotes_avaliacao 
            ADD COLUMN setor_id INTEGER;
            RAISE NOTICE 'Coluna setor_id adicionada (sem FK - tabela setores não existe)';
        END IF;
    ELSE
