-- Migration 606: Atualizar função calcular_elegibilidade_lote para usar arquitetura segregada
-- Data: 2026-02-08
-- Depende: 605_remove_obsolete_fk_columns_from_funcionarios.sql
-- CRÍTICO: A função precisa usar funcionarios_clinicas em vez de empresa_id direto

BEGIN;

-- Recriar função calcular_elegibilidade_lote para usar tabela intermediária
...
BEGIN
  ...
  WHERE
    ...
    AND (
      ...
      OR
      -- Índice está atrasado MAS não tem avaliação concluída recente
      (
      ...
      OR
      -- Nunca completou nenhuma avaliação (apenas inativadas)
...
'Calcula quais funcionários devem ser incluídos no próximo lote de avaliação para uma empresa.
ARQUITETURA SEGREGADA: Usa funcionarios_clinicas em vez de empresa_id direto.';

-- Recriar função calcular_elegibilidade_lote_tomador (equivalente para entidades - nomenclatura atualizada)
...
'Calcula quais funcionários devem ser incluídos no próximo lote de avaliação para uma entidade (tomador).
...
-- Remover função legada
DROP FUNCTION IF EXISTS calcular_elegibilidade_lote_contratante(INTEGER, INTEGER) CASCADE;

COMMIT;

SELECT 'Migration 606 aplicada com sucesso - funções atualizadas para arquitetura segregada com nomenclatura tomador' AS status;
