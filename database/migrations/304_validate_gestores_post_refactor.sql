...
-- 1. VALIDAR GESTORES: Todos em entidades_senhas devem ter contratante_id
DO $$
DECLARE
...
BEGIN
    ...
    IF v_invalid_count > 0 THEN
        RAISE WARNING 'Encontrados % gestores sem contratante_id em entidades_senhas', v_invalid_count;
...
BEGIN
    ...
    IF v_invalid_lotes > 0 THEN
        RAISE WARNING 'Existem % lotes com liberado_por invalido (nao esta em entidades_senhas)', v_invalid_lotes;
...
CREATE INDEX IF NOT EXISTS idx_entidades_senhas_contratante_id 
...
'CPF do gestor que liberou o lote. Referencia entidades_senhas(cpf) para gestores de entidade ou RH de clínica';

COMMENT ON CONSTRAINT lotes_avaliacao_liberado_por_fkey ON lotes_avaliacao IS 
'FK para entidades_senhas - gestores não estão em funcionarios após refatoração';

COMMIT;

-- VALIDAÇÃO FINAL
SELECT 
    'entidades_senhas' as tabela,
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE liberado_por IN (SELECT cpf FROM entidades_senhas)) as liberado_por_valido,
    COUNT(*) FILTER (WHERE liberado_por NOT IN (SELECT cpf FROM entidades_senhas)) as liberado_por_invalido
