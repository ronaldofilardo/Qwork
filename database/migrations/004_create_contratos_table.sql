...
-- Nota: Referencia tabela 'entidades' (renomeada de 'contratantes' na Migration 420)

DO $$
DECLARE
    v_entidades_table_name TEXT;
BEGIN
  -- Detectar nome correto da tabela (entidades ou contratantes)
  ...
  ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contratantes') THEN
    v_entidades_table_name := 'contratantes';
  ELSE
    RAISE EXCEPTION 'Tabela entidades ou contratantes não encontrada';
  END IF;
