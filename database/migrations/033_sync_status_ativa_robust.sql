...
FOR EACH ROW EXECUTE FUNCTION contratantes_sync_status_ativa_robust();
...
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE NOTICE 'Corrigidos % registros inconsistentes em contratantes', v_count;
    ELSE
        RAISE NOTICE 'Nenhuma inconsistência encontrada em contratantes';
    END IF;
END $$;
