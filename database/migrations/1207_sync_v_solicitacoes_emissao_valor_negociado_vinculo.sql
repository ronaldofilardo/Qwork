-- Migration: 1207
-- Description: Adicionar valor_negociado_vinculo à view v_solicitacoes_emissao
-- Motivo: Exibir no suporte o valor negociado (registrado via drawer) para custo fixo

-- Dropar a view existente (sem CASCADE para evitar dependências)
DROP VIEW IF EXISTS public.v_solicitacoes_emissao CASCADE;

-- Recriar a view com o novo campo valor_negociado_vinculo
...
COMMENT ON VIEW public.v_solicitacoes_emissao IS 'View para admin gerenciar solicitações de emissão. comissoes_geradas_count: total de comissões criadas para o lote (inclui provisionadas futuras). comissoes_ativas_count: comissões com parcela_confirmada_em IS NOT NULL (parcela efetivamente paga). Diferença = parcelas futuras provisionadas aguardando pagamento. valor_negociado_vinculo: valor registrado no vínculo via drawer (para custo fixo).';
