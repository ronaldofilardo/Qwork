-- Migration: 1210
-- Date: 2026-04-16
-- Description: Adiciona r.percentual_comissao_comercial à view v_solicitacoes_emissao.
--   Necessário para exibir o % comercial no card de cobrança do suporte.
...
CREATE VIEW public.v_solicitacoes_emissao AS
 SELECT la.id AS lote_id,
  ...
  GROUP BY la.id, e.nome, e.id, c.nome, c.id, c.entidade_id, ent.nome, u.nome, u.cpf,
           ...
           r.percentual_comissao_comercial, r.modelo_comissionamento,
...
COMMENT ON VIEW public.v_solicitacoes_emissao IS
  'View consolidada (1210) para admin/suporte gerenciar solicitações de emissão.'
  ' Adiciona representante_percentual_comissao_comercial para exibição no card de cobrança.';
