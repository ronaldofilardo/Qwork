...
-- Data: 2025-12-26
-- Objetivo: Garantir que novos tomadores sejam criados inativos por padrão

-- Alterar default de ativa para false (consistente com ativação após confirmação de pagamento)
ALTER TABLE tomadores ALTER COLUMN ativa SET DEFAULT false;

COMMENT ON COLUMN tomadores.ativa IS 
'Indica se o contratante está ativo no sistema. DEFAULT false - ativação ocorre APENAS após confirmação de pagamento.';

-- Verificar se há tomadores ativos sem pagamento confirmado
DO $$
DECLARE
...
BEGIN
  ...
  IF total_inconsistentes > 0 THEN
    RAISE NOTICE 'ATENÇÃO: Encontrados % tomadores ativos sem pagamento confirmado. Execute fn_corrigir_inconsistencias_tomadores() para corrigir.', total_inconsistentes;
  ELSE
