-- ATUALIZAÇÃO DA FUNÇÃO detectar_anomalias_indice
-- Versão corrigida que retorna os campos esperados pelo frontend

-- Primeiro, dropar a função existente
DROP FUNCTION IF EXISTS detectar_anomalias_indice (INTEGER);

-- Recriar com a assinatura correta
...
-- Anomalia 1: Nunca avaliado (>6 meses) OU teve avaliações liberadas mas nunca concluiu nenhuma
  SELECT
    ...
    'NUNCA_AVALIADO'::VARCHAR(50) AS tipo_anomalia,
    CASE
  ...
  WHERE
    ...
    SELECT
      ...
      'ALTA'::VARCHAR(20) AS severidade,
      'MAIS_DE_1_ANO_SEM_AVALIACAO'::VARCHAR(50) AS tipo_anomalia,
    ...
    SELECT
      ...
      f.data_ultimo_lote,
      EXTRACT(DAY FROM NOW() - f.data_ultimo_lote)::INTEGER AS dias_desde_ultima_avaliacao,
      'CRÍTICA'::VARCHAR(20) AS severidade,
      'MAIS_DE_2_ANOS_SEM_AVALIACAO'::VARCHAR(50) AS tipo_anomalia,
  ...
  SELECT
    anomalias.cpf AS funcionario_cpf,
    anomalias.nome AS funcionario_nome,
    anomalias.setor,
    anomalias.indice_avaliacao,
    anomalias.data_ultimo_lote,
    anomalias.dias_desde_ultima_avaliacao,
    anomalias.severidade,
    anomalias.categoria_anomalia AS tipo_anomalia,
    anomalias.mensagem AS detalhes
  FROM anomalias
  ORDER BY
    CASE anomalias.severidade
      WHEN 'CRÍTICA' THEN 1
      WHEN 'ALTA' THEN 2
      WHEN 'MÉDIA' THEN 3
      ELSE 4
...
COMMENT ON FUNCTION detectar_anomalias_indice (INTEGER) IS 'Detecta anomalias no sistema de avaliação com campos compatíveis com o frontend';