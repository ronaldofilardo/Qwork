 ...
 * (valor × (1 − percRep/100)) é inferior ao custo mínimo do tipo.
 */
export function calcularRequerAprovacao(
  ...
  const valorQWork = valorNegociado * (1 - percRep / 100);
 ...
 *
 * percentualRep é aplicado diretamente sobre o valor negociado.
 * Se valorQWork < custo por avaliação → abaixoCusto=true (requer aprovação).
 */
...
export interface ValoresComissaoCustoFixo {
  ...
  valorRep: number;
  /** Valor líquido que fica para o QWork (R$): margem */
  valorQWork: number;
 ...
 * ao menos CUSTO_POR_AVALIACAO[tipo].
 */
export function minimoCustoFixoPorEval(tipoCliente: TipoCliente): number {
  return CUSTO_POR_AVALIACAO[tipoCliente];
}
