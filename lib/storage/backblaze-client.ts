...
export async function getPresignedUrl(
  ...
  console.log(
    `[BACKBLAZE:getPresignedUrl] Config: endpoint=${config.endpoint}, bucket=${config.bucket}, region=${config.region}`
  );
  ...
  console.log(
    `[BACKBLAZE:getPresignedUrl] URL gerada: ${url.substring(0, 120)}...`
  );
  return url;
}
