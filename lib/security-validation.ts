...
export async function validateSessionIntegrity(session: Session): Promise<{
  ...
  try {
    ...
    if (user.perfil !== session.perfil) {
      issues.push(`Perfil inconsistente: sessão=${session.perfil}, banco=${user.perfil}`)
    }
    ...
    if (user.perfil === 'rh' || user.perfil === 'funcionario') {
      if (!user.clinica_id) {
        ...
        issues.push(`clinica_id inconsistente: sessão=${session.clinica_id}, banco=${user.clinica_id}`)
      }
    }
    ...
    if (user.perfil === 'funcionario' && user.empresa_id) {
      ...
      if (empresaResult.rows.length > 0) {
        ...
        if (empresaClinicaId !== user.clinica_id) {
          issues.push(`Empresa ${user.empresa_id} não pertence à clínica ${user.clinica_id} do funcionário`)
        }
      }
    }
    ...
    issues.push(`Erro ao validar sessão: ${error instanceof Error ? error.message : String(error)}`)
    return { isValid: false, issues }
  }
}
  ...
  try {
    switch (resourceType) {
      ...
      case 'funcionario':
        ...
        if (session.perfil === 'funcionario' && session.cpf === resourceId.toString().padStart(11, '0')) {
          return { hasAccess: true }
        }
    ...
    return { hasAccess: false, reason: `Erro na validação: ${error instanceof Error ? error.message : String(error)}` }
  }
}
  ...
  try {
    ...
    if (session.perfil === 'rh' || session.perfil === 'funcionario') {
      ...
      const [empresasAcessiveis, empresasClinica] = await Promise.all([
        query('SELECT COUNT(*) as count FROM empresas_clientes WHERE clinica_id = $1', [session.clinica_id]),
        query('SELECT COUNT(*) as count FROM empresas_clientes')
      ])

      if (parseInt(empresasAcessiveis.rows[0].count) === 0 && parseInt(empresasClinica.rows[0].count) > 0) {
        anomalies.push({
    ...
    if (session.clinica_id) {
      const clinicaExists = await query('SELECT COUNT(*) as count FROM clinicas WHERE id = $1', [session.clinica_id])
