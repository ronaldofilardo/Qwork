...
COMMENT ON FUNCTION public.audit_lote_change() IS 'Trigger de auditoria para lotes com validação rigorosa de user_cpf (11 dígitos ou NULL)';
...
COMMENT ON FUNCTION public.audit_lote_status_change() IS 'Registra mudanças de status do lote em audit_logs';
...
--
-- Name: contratantes_sync_status_ativa_robust(); Type: FUNCTION; Schema: public; Owner: neondb_owner
--
...
ALTER FUNCTION public.contratantes_sync_status_ativa_robust() OWNER TO neondb_owner;

--
-- Name: FUNCTION contratantes_sync_status_ativa_robust(); Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON FUNCTION public.contratantes_sync_status_ativa_robust() IS 'Sincroniza status_ativa com status de forma robusta';


--
-- Name: corrigir_status_avaliacoes(); Type: FUNCTION; Schema: public; Owner: neondb_owner
--

CREATE FUNCTION public.corrigir_status_avaliacoes() RETURNS TABLE(avaliacao_id integer, status_anterior text, status_novo text, total_respostas bigint)
    LANGUAGE plpgsql SECURITY DEFINER
...
ALTER FUNCTION public.corrigir_status_avaliacoes() OWNER TO neondb_owner;
...
COMMENT ON FUNCTION public.fn_reservar_id_laudo_on_lote_insert() IS 'Reserva ID do laudo igual ao lote no momento da inserção (colunas mínimas: id, lote_id)';
...
COMMENT ON FUNCTION public.fn_validar_transicao_status_lote() IS 'Valida transições de status permitidas para lotes';
...
CREATE FUNCTION public.limpar_auditoria_laudos_antiga() RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
...
COMMENT ON FUNCTION public.limpar_auditoria_laudos_antiga() IS 'Remove registros de auditoria de laudos com mais de 90 dias';
...
COMMENT ON FUNCTION public.prevent_lote_mutation_during_emission() IS 'Protege lote durante processo de emissão';
...
COMMENT ON FUNCTION public.prevent_lote_status_change_after_emission() IS 'Impede mudança de status do lote após laudo emitido';
...
COMMENT ON FUNCTION public.prevent_modification_after_emission() IS 'Impede modificação/exclusão de avaliações após emissão do laudo';
...
COMMENT ON FUNCTION public.trg_recalc_lote_on_avaliacao_change() IS 'Wrapper que chama fn_recalcular_status_lote_on_avaliacao_update corretamente';
...
COMMENT ON COLUMN public.auditoria_laudos.tipo_solicitante IS 'Tipo do solicitante: rh ou gestor_entidade';
...
COMMENT ON COLUMN public.empresas_clientes.clinica_id IS 'ID da clínica (NULL se empresa pertence a entidade contratante)';
...
--
-- Name: COLUMN empresas_clientes.contratante_id; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON COLUMN public.empresas_clientes.contratante_id IS 'ID do contratante (para empresas que pertencem a uma entidade contratante)';
...
CREATE TABLE public.funcionarios (
    ...
    ultimo_lote_codigo character varying(255),
...
COMMENT ON COLUMN public.funcionarios.ultimo_lote_codigo IS 'Código do último lote em que o funcionário participou (desnormalizado para performance)';


--
-- Name: equipe_administrativa; Type: VIEW; Schema: public; Owner: neondb_owner
--
...
CREATE VIEW public.equipe_administrativa AS
 SELECT f.cpf,
  ...
  WHERE ((f.perfil)::text = ANY ((ARRAY['admin'::character varying, 'master'::character varying, 'emissor'::character varying])::text[]))
  ORDER BY f.perfil, f.nome;


ALTER VIEW public.equipe_administrativa OWNER TO neondb_owner;

--
-- Name: VIEW equipe_administrativa; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON VIEW public.equipe_administrativa IS 'Lista de funcionários administrativos (admin, master, emissor)';
...
--
-- Name: funcionarios_operacionais; Type: VIEW; Schema: public; Owner: neondb_owner
--
...
CREATE VIEW public.funcionarios_operacionais AS
 SELECT f.cpf,
  ...
  WHERE ((f.perfil)::text = ANY ((ARRAY['funcionario'::character varying, 'gestor_entidade'::character varying])::text[]))
  ORDER BY f.empresa_id, f.nome;


ALTER VIEW public.funcionarios_operacionais OWNER TO neondb_owner;

--
-- Name: VIEW funcionarios_operacionais; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON VIEW public.funcionarios_operacionais IS 'Lista de funcionários operacionais (funcionario, gestor_entidade)';
...
CREATE VIEW public.gestores AS
 SELECT f.cpf,
   ...
   FROM ((public.funcionarios f
     LEFT JOIN public.empresas_clientes ec ON ((f.empresa_id = ec.id)))
     LEFT JOIN public.contratantes c ON ((f.contratante_id = c.id)))
  WHERE ((f.perfil)::text = ANY ((ARRAY['gestor_entidade'::character varying, 'rh'::character varying])::text[]))
  ORDER BY f.perfil, f.nome;
...
--
-- Name: VIEW gestores; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON VIEW public.gestores IS 'Lista de gestores (gestor_entidade, rh)';
...
CREATE TABLE public.laudos (
    ...
    hash_pdf character varying(64),
    job_id bigint,
    arquivo_remoto_provider character varying(255),
    arquivo_remoto_bucket character varying(255),
    arquivo_remoto_key text,
    arquivo_remoto_url text,
...
--
-- Name: COLUMN laudos.job_id; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON COLUMN public.laudos.job_id IS 'ID do job de geração do laudo (referência para rastreamento)';


--
-- Name: COLUMN laudos.arquivo_remoto_provider; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON COLUMN public.laudos.arquivo_remoto_provider IS 'Provider de armazenamento remoto (s3, backblaze, etc)';


--
-- Name: COLUMN laudos.arquivo_remoto_bucket; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON COLUMN public.laudos.arquivo_remoto_bucket IS 'Bucket/container onde o arquivo está armazenado';


--
-- Name: COLUMN laudos.arquivo_remoto_key; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON COLUMN public.laudos.arquivo_remoto_key IS 'Chave/caminho do arquivo no armazenamento remoto';


--
-- Name: COLUMN laudos.arquivo_remoto_url; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON COLUMN public.laudos.arquivo_remoto_url IS 'URL completa para acesso ao arquivo remoto';
...
COMMENT ON COLUMN public.lotes_avaliacao.modo_emergencia IS 'Indica se o lote foi processado em modo emergência';
...
COMMENT ON COLUMN public.lotes_avaliacao.motivo_emergencia IS 'Motivo da ativação do modo emergência';


--
-- Name: CONSTRAINT lotes_avaliacao_status_check ON lotes_avaliacao; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON CONSTRAINT lotes_avaliacao_status_check ON public.lotes_avaliacao IS 'Status válidos para lote de avaliação - sincronizado com desenvolvimento';
...
--
-- Name: usuarios_resumo; Type: VIEW; Schema: public; Owner: neondb_owner
--
...
CREATE VIEW public.usuarios_resumo AS
 SELECT f.cpf,
   ...
   FROM (((public.funcionarios f
     LEFT JOIN public.empresas_clientes ec ON ((f.empresa_id = ec.id)))
     LEFT JOIN public.contratantes c ON ((f.contratante_id = c.id)))
     LEFT JOIN public.clinicas cl ON ((f.clinica_id = cl.id)))
  ORDER BY f.criado_em DESC;


ALTER VIEW public.usuarios_resumo OWNER TO neondb_owner;

--
-- Name: VIEW usuarios_resumo; Type: COMMENT; Schema: public; Owner: neondb_owner
--

COMMENT ON VIEW public.usuarios_resumo IS 'Resumo de todos os usuários do sistema';
...
--
-- Data for Name: _backup_fila_emissao_20260204; Type: TABLE DATA; Schema: public; Owner: neondb_owner
--
...
--
-- Data for Name: _deprecated_fila_emissao; Type: TABLE DATA; Schema: public; Owner: neondb_owner
--
...
163	\N	\N	INSERT	avaliacoes	17	\N	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record created	2026-02-04 14:38:32.94737	\N	\N
164	87545772920	\N	liberar_lote	lotes_avaliacao	21	\N	\N	177.146.165.115	\N	{"contratante_id":1,"contratante_nome":"RELEGERE","tipo":"completo","lote_id":21,"descricao":null,"data_filtro":null,"numero_ordem":6,"avaliacoes_criadas":1,"total_funcionarios":1}	2026-02-04 14:38:33.892221	\N	\N
...
165	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:39:49.522629	\N	\N
166	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:39:51.80394	\N	\N
...
156	\N	\N	lote_criado	lotes_avaliacao	20	\N	\N	\N	\N	{"status": "ativo", "lote_id": 20, "empresa_id": 1, "numero_ordem": 3}	2026-02-04 14:38:11.685257	\N	\N
157	\N	\N	INSERT	laudos	20	\N	{"id": 20, "status": "emitido", "lote_id": 20, "hash_pdf": null, "criado_em": "2026-02-04T14:38:11.685257", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T14:38:11.685257", "relatorio_lote": null, "relatorio_setor": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "hash_relatorio_individual": null}	\N	\N	Record created	2026-02-04 14:38:11.685257	\N	\N
158	\N	\N	laudo_criado	laudos	20	\N	{"status": "emitido", "lote_id": 20, "tamanho_pdf": null}	\N	\N	\N	2026-02-04 14:38:11.685257	\N	\N
159	\N	\N	INSERT	avaliacoes	16	\N	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record created	2026-02-04 14:38:13.59276	\N	\N
160	\N	\N	lote_criado	lotes_avaliacao	21	\N	\N	\N	\N	{"status": "ativo", "lote_id": 21, "empresa_id": null, "numero_ordem": 6}	2026-02-04 14:38:32.006823	\N	\N
161	\N	\N	INSERT	laudos	21	\N	{"id": 21, "status": "emitido", "lote_id": 21, "hash_pdf": null, "criado_em": "2026-02-04T14:38:32.006823", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T14:38:32.006823", "relatorio_lote": null, "relatorio_setor": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "hash_relatorio_individual": null}	\N	\N	Record created	2026-02-04 14:38:32.006823	\N	\N
162	\N	\N	laudo_criado	laudos	21	\N	{"status": "emitido", "lote_id": 21, "tamanho_pdf": null}	\N	\N	\N	2026-02-04 14:38:32.006823	\N	\N
167	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:39:55.045858	\N	\N
168	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:39:57.960541	\N	\N
169	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:01.682111	\N	\N
170	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:05.04401	\N	\N
171	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:09.914362	\N	\N
172	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:13.961562	\N	\N
173	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:22.585301	\N	\N
174	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:26.267781	\N	\N
175	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:33.76296	\N	\N
176	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:37.151109	\N	\N
177	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:42.685815	\N	\N
178	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:46.156411	\N	\N
179	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:49.392589	\N	\N
180	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:52.45498	\N	\N
181	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:40:56.278304	\N	\N
182	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:00.644645	\N	\N
183	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:05.38642	\N	\N
184	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:08.816758	\N	\N
185	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:14.175769	\N	\N
186	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:19.626508	\N	\N
187	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:22.45856	\N	\N
188	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 14:41:52.322811	\N	\N
203	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:20:09.933694	\N	\N
204	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:20:50.873487	\N	\N
205	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:20:53.384718	\N	\N
206	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:20:57.469937	\N	\N
207	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:01.336402	\N	\N
208	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:05.040935	\N	\N
209	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:13.659597	\N	\N
210	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:18.895646	\N	\N
211	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:22.521706	\N	\N
212	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:25.693223	\N	\N
213	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:32.091234	\N	\N
214	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:35.967587	\N	\N
215	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:39.863755	\N	\N
216	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:43.722386	\N	\N
217	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:47.522709	\N	\N
218	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:51.22974	\N	\N
219	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:21:56.405714	\N	\N
220	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:02.420993	\N	\N
221	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:06.099858	\N	\N
222	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:09.800343	\N	\N
223	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:13.856208	\N	\N
224	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:17.338482	\N	\N
225	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:21.214507	\N	\N
226	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:24.132601	\N	\N
227	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:27.858994	\N	\N
228	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:32.376826	\N	\N
229	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:36.080884	\N	\N
230	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:39.815042	\N	\N
231	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:43.78883	\N	\N
232	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:47.151173	\N	\N
233	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:50.788835	\N	\N
234	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:54.467613	\N	\N
235	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:22:58.883401	\N	\N
236	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:04.233269	\N	\N
237	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:09.339646	\N	\N
238	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:12.684263	\N	\N
239	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:16.321944	\N	\N
240	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:20.73624	\N	\N
241	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:24.872625	\N	\N
242	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:23:29.609906	\N	\N
243	\N	\N	UPDATE	funcionarios	28	{"id": 28, "cpf": "37845006092", "nome": "joa do suco", "ativo": true, "email": "ipopioio@ipipo.com", "setor": "iopipo", "turno": null, "escala": null, "funcao": "poiipo", "perfil": "funcionario", "criado_em": "2026-02-03T13:37:01.86638", "matricula": null, "clinica_id": 1, "empresa_id": 1, "senha_hash": "$2a$10$OH2J9OlXrhBIzgrix.Af3eFhiN4Wtrx8zKTU5PK8bAoUHAIyp8uNa", "incluido_em": "2026-02-03T13:37:01.86638", "nivel_cargo": "gestao", "inativado_em": null, "usuario_tipo": "funcionario_clinica", "atualizado_em": "2026-02-03T13:37:01.86638", "data_admissao": null, "inativado_por": null, "contratante_id": null, "data_nascimento": "1966-02-01", "data_ultimo_lote": null, "indice_avaliacao": 0, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	{"id": 28, "cpf": "37845006092", "nome": "joa do suco", "ativo": true, "email": "ipopioio@ipipo.com", "setor": "iopipo", "turno": null, "escala": null, "funcao": "poiipo", "perfil": "funcionario", "criado_em": "2026-02-03T13:37:01.86638", "matricula": null, "clinica_id": 1, "empresa_id": 1, "senha_hash": "$2a$10$OH2J9OlXrhBIzgrix.Af3eFhiN4Wtrx8zKTU5PK8bAoUHAIyp8uNa", "incluido_em": "2026-02-03T13:37:01.86638", "nivel_cargo": "gestao", "inativado_em": null, "usuario_tipo": "funcionario_clinica", "atualizado_em": "2026-02-03T13:37:01.86638", "data_admissao": null, "inativado_por": null, "contratante_id": null, "data_nascimento": "1966-02-01", "data_ultimo_lote": "2026-02-04T15:23:30.074973", "indice_avaliacao": 3, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	\N	\N	Record updated	2026-02-04 15:23:30.074973	\N	\N
244	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:24:51.202451	\N	\N
245	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:26:27.843918	\N	\N
246	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:01.287418	\N	\N
247	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:25.276305	\N	\N
248	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:27.439918	\N	\N
249	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:30.817364	\N	\N
250	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:34.114548	\N	\N
251	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:37.512871	\N	\N
252	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:56.826062	\N	\N
253	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:27:59.047128	\N	\N
254	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:28:16.061126	\N	\N
255	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:28:33.390215	\N	\N
256	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:28:36.191198	\N	\N
257	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:28:39.499275	\N	\N
258	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:29:14.557341	\N	\N
259	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:39:59.833722	\N	\N
260	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:41:01.461998	\N	\N
261	\N	\N	UPDATE	laudos	20	{"id": 20, "job_id": null, "status": "emitido", "lote_id": 20, "hash_pdf": null, "criado_em": "2026-02-04T14:38:11.685257", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T14:38:11.685257", "relatorio_lote": null, "relatorio_setor": null, "arquivo_remoto_key": null, "arquivo_remoto_url": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "arquivo_remoto_bucket": null, "arquivo_remoto_provider": null, "hash_relatorio_individual": null}	{"id": 20, "job_id": null, "status": null, "lote_id": 20, "hash_pdf": null, "criado_em": "2026-02-04T14:38:11.685257", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T14:38:11.685257", "relatorio_lote": null, "relatorio_setor": null, "arquivo_remoto_key": null, "arquivo_remoto_url": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "arquivo_remoto_bucket": null, "arquivo_remoto_provider": null, "hash_relatorio_individual": null}	\N	\N	Record updated	2026-02-04 15:43:08.982738	\N	\N
262	\N	\N	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:43:18.377678	\N	\N
267	\N	\N	UPDATE	laudos	21	{"id": 21, "job_id": null, "status": "emitido", "lote_id": 21, "hash_pdf": null, "criado_em": "2026-02-04T14:38:32.006823", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T14:38:32.006823", "relatorio_lote": null, "relatorio_setor": null, "arquivo_remoto_key": null, "arquivo_remoto_url": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "arquivo_remoto_bucket": null, "arquivo_remoto_provider": null, "hash_relatorio_individual": null}	{"id": 21, "job_id": null, "status": null, "lote_id": 21, "hash_pdf": null, "criado_em": "2026-02-04T14:38:32.006823", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T14:38:32.006823", "relatorio_lote": null, "relatorio_setor": null, "arquivo_remoto_key": null, "arquivo_remoto_url": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "arquivo_remoto_bucket": null, "arquivo_remoto_provider": null, "hash_relatorio_individual": null}	\N	\N	Record updated	2026-02-04 15:44:38.56882	\N	\N
268	00000000000	system	UPDATE	avaliacoes	16	{"id": 16, "envio": null, "inicio": "2026-02-04T14:38:12.704", "status": "iniciada", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:13.59276", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 16, "envio": "2026-02-04T15:44:38.56882", "inicio": "2026-02-04T14:38:12.704", "status": "concluida", "lote_id": 20, "criado_em": "2026-02-04T14:38:13.59276", "grupo_atual": 1, "concluida_em": "2026-02-04T15:44:38.56882", "inativada_em": null, "atualizado_em": "2026-02-04T15:44:38.56882", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 15:44:38.56882	\N	\N
269	00000000000	\N	lote_atualizado	lotes_avaliacao	20	\N	\N	127.0.0.1	\N	{"status": "concluido", "lote_id": 20, "mudancas": {"status_novo": "concluido", "status_anterior": "ativo"}, "emitido_em": null, "enviado_em": null, "processamento_em": null}	2026-02-04 15:44:38.56882	\N	\N
270	00000000000	\N	status_alterado	lotes_avaliacao	20	\N	\N	127.0.0.1	\N	{"lote_id": 20, "status_novo": "concluido", "status_anterior": "ativo"}	2026-02-04 15:44:38.56882	\N	\N
271	\N	\N	UPDATE	avaliacoes	13	{"id": 13, "envio": null, "inicio": "2026-02-03T13:37:18.101", "status": "iniciada", "lote_id": 10, "criado_em": "2026-02-03T13:37:18.978955", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:37:18.978955", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 13, "envio": null, "inicio": "2026-02-03T13:37:18.101", "status": "iniciada", "lote_id": 10, "criado_em": "2026-02-03T13:37:18.978955", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:37:18.978955", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:05:56.536381	\N	\N
272	\N	\N	UPDATE	avaliacoes	13	{"id": 13, "envio": null, "inicio": "2026-02-03T13:37:18.101", "status": "iniciada", "lote_id": 10, "criado_em": "2026-02-03T13:37:18.978955", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:37:18.978955", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	{"id": 13, "envio": null, "inicio": "2026-02-03T13:37:18.101", "status": "iniciada", "lote_id": 10, "criado_em": "2026-02-03T13:37:18.978955", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:37:18.978955", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:06:37.075226	\N	\N
273	\N	\N	UPDATE	avaliacoes	8	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:08:03.992019	\N	\N
274	\N	\N	UPDATE	avaliacoes	8	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:08:32.135051	\N	\N
275	\N	\N	UPDATE	avaliacoes	8	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:26:34.010046	\N	\N
276	\N	\N	UPDATE	avaliacoes	8	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:28:13.862039	\N	\N
277	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:29:36.465411	\N	\N
278	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:56:26.982586	\N	\N
279	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:56:29.969646	\N	\N
280	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:56:33.61598	\N	\N
281	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 17:56:36.590618	\N	\N
282	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:08:34.454657	\N	\N
283	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:22:03.132432	\N	\N
284	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:22:06.193027	\N	\N
285	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:38:53.771793	\N	\N
286	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:38:56.360659	\N	\N
287	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:38:59.16657	\N	\N
288	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:02.193456	\N	\N
289	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:05.403074	\N	\N
290	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:08.692428	\N	\N
291	\N	\N	UPDATE	avaliacoes	8	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 8, "envio": null, "inicio": "2026-02-03T13:04:20.547", "status": "iniciada", "lote_id": 7, "criado_em": "2026-02-03T13:04:21.412592", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-03T13:04:21.412592", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:29.539526	\N	\N
292	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:47.231101	\N	\N
293	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:50.771446	\N	\N
294	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:54.051141	\N	\N
295	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:39:57.54663	\N	\N
296	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:02.451454	\N	\N
297	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:07.378219	\N	\N
298	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:10.555157	\N	\N
299	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:13.567846	\N	\N
300	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:17.065607	\N	\N
301	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:20.174313	\N	\N
302	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:23.477247	\N	\N
303	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:26.138426	\N	\N
304	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:29.313115	\N	\N
305	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:32.748986	\N	\N
306	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:35.851392	\N	\N
307	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:41.873381	\N	\N
308	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:45.128821	\N	\N
309	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:49.291889	\N	\N
310	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:53.124557	\N	\N
311	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:40:57.607927	\N	\N
312	\N	\N	UPDATE	funcionarios	25	{"id": 25, "cpf": "52821297017", "nome": "dfsiopi adpipoipo", "ativo": true, "email": "oiuoiu@uiuooiu.com", "setor": "uoiuoiu", "turno": null, "escala": null, "funcao": "oiuoiuoiu", "perfil": "funcionario", "criado_em": "2026-02-03T13:03:58.168595", "matricula": null, "clinica_id": null, "empresa_id": null, "senha_hash": "$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m", "incluido_em": "2026-02-03T13:03:58.168595", "nivel_cargo": "operacional", "inativado_em": null, "usuario_tipo": "funcionario_entidade", "atualizado_em": "2026-02-03T13:03:58.168595", "data_admissao": null, "inativado_por": null, "contratante_id": 1, "data_nascimento": "2000-02-01", "data_ultimo_lote": null, "indice_avaliacao": 0, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	{"id": 25, "cpf": "52821297017", "nome": "dfsiopi adpipoipo", "ativo": true, "email": "oiuoiu@uiuooiu.com", "setor": "uoiuoiu", "turno": null, "escala": null, "funcao": "oiuoiuoiu", "perfil": "funcionario", "criado_em": "2026-02-03T13:03:58.168595", "matricula": null, "clinica_id": null, "empresa_id": null, "senha_hash": "$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m", "incluido_em": "2026-02-03T13:03:58.168595", "nivel_cargo": "operacional", "inativado_em": null, "usuario_tipo": "funcionario_entidade", "atualizado_em": "2026-02-03T13:03:58.168595", "data_admissao": null, "inativado_por": null, "contratante_id": 1, "data_nascimento": "2000-02-01", "data_ultimo_lote": "2026-02-04T18:40:58.076267", "indice_avaliacao": 6, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	\N	\N	Record updated	2026-02-04 18:40:58.076267	\N	\N
313	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:51:42.109195	\N	\N
314	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:51:45.516464	\N	\N
315	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 18:51:50.01698	\N	\N
327	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:04:30.027294	\N	\N
316	\N	\N	UPDATE	funcionarios	25	{"id": 25, "cpf": "52821297017", "nome": "dfsiopi adpipoipo", "ativo": true, "email": "oiuoiu@uiuooiu.com", "setor": "uoiuoiu", "turno": null, "escala": null, "funcao": "oiuoiuoiu", "perfil": "funcionario", "criado_em": "2026-02-03T13:03:58.168595", "matricula": null, "clinica_id": null, "empresa_id": null, "senha_hash": "$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m", "incluido_em": "2026-02-03T13:03:58.168595", "nivel_cargo": "operacional", "inativado_em": null, "usuario_tipo": "funcionario_entidade", "atualizado_em": "2026-02-03T13:03:58.168595", "data_admissao": null, "inativado_por": null, "contratante_id": 1, "data_nascimento": "2000-02-01", "data_ultimo_lote": "2026-02-04T18:40:58.076267", "indice_avaliacao": 6, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	{"id": 25, "cpf": "52821297017", "nome": "dfsiopi adpipoipo", "ativo": true, "email": "oiuoiu@uiuooiu.com", "setor": "uoiuoiu", "turno": null, "escala": null, "funcao": "oiuoiuoiu", "perfil": "funcionario", "criado_em": "2026-02-03T13:03:58.168595", "matricula": null, "clinica_id": null, "empresa_id": null, "senha_hash": "$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m", "incluido_em": "2026-02-03T13:03:58.168595", "nivel_cargo": "operacional", "inativado_em": null, "usuario_tipo": "funcionario_entidade", "atualizado_em": "2026-02-03T13:03:58.168595", "data_admissao": null, "inativado_por": null, "contratante_id": 1, "data_nascimento": "2000-02-01", "data_ultimo_lote": "2026-02-04T18:51:50.748393", "indice_avaliacao": 6, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	\N	\N	Record updated	2026-02-04 18:51:50.748393	\N	\N
318	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:02:07.374191	\N	\N
319	\N	\N	lote_criado	lotes_avaliacao	1	\N	\N	\N	\N	{"status": "ativo", "lote_id": 1, "empresa_id": null, "numero_ordem": 7}	2026-02-04 19:03:34.053286	\N	\N
320	\N	\N	INSERT	laudos	1	\N	{"id": 1, "job_id": null, "status": "emitido", "lote_id": 1, "hash_pdf": null, "criado_em": "2026-02-04T19:03:34.053286", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T19:03:34.053286", "relatorio_lote": null, "relatorio_setor": null, "arquivo_remoto_key": null, "arquivo_remoto_url": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "arquivo_remoto_bucket": null, "arquivo_remoto_provider": null, "hash_relatorio_individual": null}	\N	\N	Record created	2026-02-04 19:03:34.053286	\N	\N
321	\N	\N	laudo_criado	laudos	1	\N	{"status": "emitido", "lote_id": 1, "tamanho_pdf": null}	\N	\N	\N	2026-02-04 19:03:34.053286	\N	\N
322	\N	\N	INSERT	avaliacoes	18	\N	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record created	2026-02-04 19:03:35.055334	\N	\N
323	87545772920	\N	liberar_lote	lotes_avaliacao	1	\N	\N	177.146.165.115	\N	{"contratante_id":1,"contratante_nome":"RELEGERE","tipo":"completo","lote_id":1,"descricao":null,"data_filtro":null,"numero_ordem":7,"avaliacoes_criadas":1,"total_funcionarios":1}	2026-02-04 19:03:36.024136	\N	\N
324	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:04:21.64036	\N	\N
325	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:04:23.718931	\N	\N
326	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:04:26.910187	\N	\N
328	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:05:13.011812	\N	\N
329	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:17:20.01149	\N	\N
330	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:18:01.854459	\N	\N
332	\N	\N	UPDATE	avaliacoes	17	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 17, "envio": null, "inicio": "2026-02-04T14:38:32.067", "status": "iniciada", "lote_id": 21, "criado_em": "2026-02-04T14:38:32.94737", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T14:38:32.94737", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:21:35.038784	\N	\N
333	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "iniciada", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:03:35.055334", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:21:35.038784	\N	\N
337	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "em_andamento", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:23:19.468558", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "em_andamento", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:23:19.468558", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:24:44.019073	\N	\N
338	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "em_andamento", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:23:19.468558", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "em_andamento", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:23:19.468558", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:24:56.478561	\N	\N
339	\N	\N	UPDATE	avaliacoes	18	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "em_andamento", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:23:19.468558", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	{"id": 18, "envio": null, "inicio": "2026-02-04T19:03:34.169", "status": "em_andamento", "lote_id": 1, "criado_em": "2026-02-04T19:03:35.055334", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:23:19.468558", "funcionario_cpf": "52821297017", "motivo_inativacao": null}	\N	\N	Record updated	2026-02-04 19:25:59.838261	\N	\N
340	\N	\N	UPDATE	funcionarios	25	{"id": 25, "cpf": "52821297017", "nome": "dfsiopi adpipoipo", "ativo": true, "email": "oiuoiu@uiuooiu.com", "setor": "uoiuoiu", "turno": null, "escala": null, "funcao": "oiuoiuoiu", "perfil": "funcionario", "criado_em": "2026-02-03T13:03:58.168595", "matricula": null, "clinica_id": null, "empresa_id": null, "senha_hash": "$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m", "incluido_em": "2026-02-03T13:03:58.168595", "nivel_cargo": "operacional", "inativado_em": null, "usuario_tipo": "funcionario_entidade", "atualizado_em": "2026-02-03T13:03:58.168595", "data_admissao": null, "inativado_por": null, "contratante_id": 1, "data_nascimento": "2000-02-01", "data_ultimo_lote": "2026-02-04T18:51:50.748393", "indice_avaliacao": 6, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	{"id": 25, "cpf": "52821297017", "nome": "dfsiopi adpipoipo", "ativo": true, "email": "oiuoiu@uiuooiu.com", "setor": "uoiuoiu", "turno": null, "escala": null, "funcao": "oiuoiuoiu", "perfil": "funcionario", "criado_em": "2026-02-03T13:03:58.168595", "matricula": null, "clinica_id": null, "empresa_id": null, "senha_hash": "$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m", "incluido_em": "2026-02-03T13:03:58.168595", "nivel_cargo": "operacional", "inativado_em": null, "usuario_tipo": "funcionario_entidade", "atualizado_em": "2026-02-03T13:03:58.168595", "data_admissao": null, "inativado_por": null, "contratante_id": 1, "data_nascimento": "2000-02-01", "data_ultimo_lote": "2026-02-04T19:26:00.319216", "indice_avaliacao": 7, "ultimo_lote_codigo": null, "ultima_avaliacao_id": null, "ultima_avaliacao_status": null, "ultimo_motivo_inativacao": null, "ultima_avaliacao_data_conclusao": null}	\N	\N	Record updated	2026-02-04 19:26:00.319216	\N	\N
341	\N	\N	lote_criado	lotes_avaliacao	2	\N	\N	\N	\N	{"status": "ativo", "lote_id": 2, "empresa_id": 1, "numero_ordem": 4}	2026-02-04 19:49:58.79625	\N	\N
342	\N	\N	INSERT	laudos	2	\N	{"id": 2, "job_id": null, "status": "emitido", "lote_id": 2, "hash_pdf": null, "criado_em": "2026-02-04T19:49:58.79625", "emitido_em": null, "enviado_em": null, "emissor_cpf": null, "observacoes": null, "atualizado_em": "2026-02-04T19:49:58.79625", "relatorio_lote": null, "relatorio_setor": null, "arquivo_remoto_key": null, "arquivo_remoto_url": null, "hash_relatorio_lote": null, "hash_relatorio_setor": null, "relatorio_individual": null, "arquivo_remoto_bucket": null, "arquivo_remoto_provider": null, "hash_relatorio_individual": null}	\N	\N	Record created	2026-02-04 19:49:58.79625	\N	\N
343	\N	\N	laudo_criado	laudos	2	\N	{"status": "emitido", "lote_id": 2, "tamanho_pdf": null}	\N	\N	\N	2026-02-04 19:49:58.79625	\N	\N
344	\N	\N	INSERT	avaliacoes	19	\N	{"id": 19, "envio": null, "inicio": "2026-02-04T19:49:59.807", "status": "iniciada", "lote_id": 2, "criado_em": "2026-02-04T19:50:00.695716", "grupo_atual": 1, "concluida_em": null, "inativada_em": null, "atualizado_em": "2026-02-04T19:50:00.695716", "funcionario_cpf": "37845006092", "motivo_inativacao": null}	\N	\N	Record created	2026-02-04 19:50:00.695716	\N	\N
\.
...
21	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"tipo_contratante": "entidade"}	8ba244d55ff4e0a2f4009fbd89423c923c185ce4a4be5158a3c737c119dbc30e	2026-02-04 13:51:05.298561
22	login	2	login_sucesso	\N	\N	04703084945	rh	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"clinica_id": 1, "tipo_contratante": "clinica"}	bcd9e1866b47715e7e2b838e33a3c86a91cd2e8ecceb604d30d7e290cdc8200e	2026-02-04 13:52:16.275636
23	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0	\N	{"tipo_contratante": "entidade"}	e714fdf572aff09b5b5a004dc8cd5865c1a33dff8b89cd5b39bd68b319d81c26	2026-02-04 13:54:49.194362
24	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0	\N	{"tipo_contratante": "entidade"}	5fc5c616449541d5353d2d2ed29bc46f33f495e664d0dbe182193519e8dabeea	2026-02-04 17:27:58.704592
25	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"tipo_contratante": "entidade"}	94ab7c48534d008a942f185ad535a0a3df013f286c7dd6477ad65ed48b78d697	2026-02-04 17:55:25.971163
26	login	2	login_sucesso	\N	\N	04703084945	rh	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0	\N	{"clinica_id": 1, "tipo_contratante": "clinica"}	b545d053350d1a8720b142e317be7e8c80d880c77abea3f59e6709179a70cf03	2026-02-04 18:03:35.252418
27	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"tipo_contratante": "entidade"}	0c818a8915fbacf9094df5a78e2ff4489aba4a4a30a0ff595dfe07f70be8fe11	2026-02-04 18:22:29.862716
28	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0	\N	{"tipo_contratante": "entidade"}	6966107aa14197521b4a9140e1e9a82dd6cb4fad355029c79e816dcf96e38c94	2026-02-04 18:23:16.531423
29	login	2	login_sucesso	\N	\N	04703084945	rh	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"clinica_id": 1, "tipo_contratante": "clinica"}	902553917719c7e0ce3290b50b7259304aa4d090c2537da9737d7a1e2746f9e9	2026-02-04 18:26:58.062346
30	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"tipo_contratante": "entidade"}	bed5a8c60386ac52d8f907551b1a88642cd9aaab57e4ba910daa8c85fce5e348	2026-02-04 18:38:08.800137
31	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0	\N	{"tipo_contratante": "entidade"}	1fdae1440d344564d4b18d9ffc7321bb4b53f81ceada77482aa9c313b19b2dc0	2026-02-04 18:42:55.715895
32	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"tipo_contratante": "entidade"}	065c6a7fb5dcc33d2ef6557cf9dffbb5e29db0b73c6e4a81c486d9ef8506f6fa	2026-02-04 18:51:09.107306
33	login	1	login_sucesso	\N	\N	87545772920	gestor_entidade	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:147.0) Gecko/20100101 Firefox/147.0	\N	{"tipo_contratante": "entidade"}	78a19503262441c8b2a6e46d95690cba399ddb13733fe00346787e3770c151f9	2026-02-04 19:41:09.161024
34	login	2	login_sucesso	\N	\N	04703084945	rh	177.146.165.115	Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/144.0.0.0 Safari/537.36	\N	{"clinica_id": 1, "tipo_contratante": "clinica"}	7852a312692a9d487a8d7c76de2e6d55618186ac178e4927c93835705e3c23ff	2026-02-04 19:49:29.122834
\.
...
1	3	\N	87545772920	Ronaldo Fjkljlk	solicitacao_manual	pendente	177.146.165.115	Solicitação manual de emissão por gestor_entidade - Lote 001-030226	2026-02-03 16:55:40.976521	\N	\N	0	\N
2	11	\N	08453792917	jaime tavares	solicitacao_manual	pendente	201.1.72.76	Solicitação manual de emissão por gestor_entidade - Lote 009-030226	2026-02-03 17:43:59.813909	\N	\N	0	\N
3	3	\N	\N	\N	solicitar_emissao	pendente	\N	\N	2026-02-03 16:55:41.249185	87545772920	gestor_entidade	0	\N
4	11	\N	\N	\N	solicitar_emissao	pendente	\N	\N	2026-02-03 17:44:00.065878	08453792917	gestor_entidade	0	\N
\.
...
4ae1adea-a285-4250-8ca8-b0e54142f13f	16	20	-1	rh	rtwtewetwetwewtew	0	2026-02-04 15:20:10.260941+00
f5f5ec0e-386c-415c-94fc-a9d9385815fb	17	21	1	gestor_entidade	dfafaffasafs	22	2026-02-04 15:27:01.52308+00
598ac611-280f-4f31-93ae-0db3cc256e62	8	7	1	gestor_entidade	dzffafaffasfsa	0	2026-02-04 18:39:29.774579+00
454140ca-18c7-4bbc-a015-9638abf94277	18	1	1	gestor_entidade	4645646546546	3	2026-02-04 19:24:44.26493+00
\.
...
16	37845006092	2026-02-04 14:38:12.704	2026-02-04 15:44:38.56882	concluida	1	2026-02-04 14:38:13.59276	2026-02-04 15:44:38.56882	20	\N	\N	2026-02-04 15:44:38.56882
...
17	52821297017	2026-02-04 14:38:32.067	2026-02-04 18:40:51.492323	concluida	1	2026-02-04 14:38:32.94737	2026-02-04 19:23:19.468558	21	\N	\N	\N
...
18	52821297017	2026-02-04 19:03:34.169	\N	em_andamento	1	2026-02-04 19:03:35.055334	2026-02-04 19:23:19.468558	1	\N	\N	\N
19	37845006092	2026-02-04 19:49:59.807	\N	iniciada	1	2026-02-04 19:50:00.695716	2026-02-04 19:50:00.695716	2	\N	\N	\N
...
COPY public.empresas_clientes (id, nome, cnpj, email, telefone, endereco, cidade, estado, cep, ativa, clinica_id, criado_em, atualizado_em, representante_nome, representante_fone, representante_email, responsavel_email, contratante_id) FROM stdin;
1	Empresa CM01	46493500000103	dfdsf@ffass.com	(64) 87897-9876	Rua jdfj lk2342	uoiuouio	UI	45678786	t	1	2026-02-03 12:45:54.683638	2026-02-03 12:45:54.683638	dsjpjp pipoippi	78464654656	dfdfdf@dsgdsgs.com	\N	\N
2	empresa cm 02	82232812000127	dffdsfds@dfsdsf.com	(67) 89454-6546	rua ldjfaslk oio 8809	uouioui	PU	46578789	t	1	2026-02-03 13:05:32.722329	2026-02-03 13:05:32.722329	cleid dpaupoip	79465432455	dffddf@dffdfds.com	\N	\N
3	XUZ	03790617000146	XUZ@gmail.com	(10) 10101-0100	Rua Francisco Raitani, 10, 6482	Curitiba	\N	81110-070	t	2	2026-02-03 18:03:47.929798	2026-02-03 18:03:47.929798	empresa 1	41988138181	chimarraobrasileiro@gmail.com	\N	\N
\.
...
27	16985430007	Mariana Maria	Operacional	Coordenadora	rolnk123132l@jijij.com	$2a$10$xz2b0qOa4fISFBN1wvLwwOJVTqzFWzwV6UBCzLV14hI1gCRU4s5JK	funcionario	t	2026-02-03 13:05:57.327759	2026-02-03 13:05:57.327759	1	2	\N	\N	\N	gestao	1974-10-24	2026-02-03 13:05:57.327759	\N	\N	\N	\N	\N	\N	\N	0	\N	\N	funcionario_clinica	\N
29	53051173991	Emissor Teste QWork	\N	\N	emissor@qwork.com.br	$2a$10$ez.cvULSRPa0CE3QugnWQeMFL2qMy9OF.lz2EW/s.cJ0Hv.2LGr7G	emissor	t	2026-02-03 15:09:53.311683	2026-02-03 15:09:53.311683	\N	\N	\N	\N	\N	\N	\N	2026-02-03 15:09:53.311683	\N	\N	\N	\N	\N	\N	\N	0	\N	\N	emissor	\N
34	01617198056	Jaiminho uoiuoiu	Operacional	Coordenadora	rolnk123132l@huhuhuj.com	$2a$10$b4NAqYhigjSU6qgsQV1gDuG1nnczw4N1hFFY5QxxLXt4p9Z7f0GTq	funcionario	t	2026-02-03 17:36:30.548672	2026-02-03 17:40:52.968392	\N	\N	\N	\N	\N	gestao	1974-10-24	2026-02-03 17:36:30.548672	\N	\N	\N	15	2026-02-03 17:40:52.968392	concluida	\N	5	2026-02-03 17:40:53.451452	3	funcionario_entidade	\N
33	59127761070	Jaiemx o1	Administrativo	Analista	joao.24@empalux.com.br	$2a$10$5MnxF0O64hhDKqA3j.rpeOCcGTTRoksEY860vwqqkjBg1qnfH6uH.	funcionario	t	2026-02-03 17:36:29.450464	2026-02-03 17:41:34.774696	\N	\N	\N	\N	\N	operacional	2010-12-12	2026-02-03 17:36:29.450464	\N	\N	\N	14	2026-02-03 17:41:34.774696	concluida	\N	5	2026-02-03 17:41:35.261523	3	funcionario_entidade	\N
28	37845006092	joa do suco	iopipo	poiipo	ipopioio@ipipo.com	$2a$10$OH2J9OlXrhBIzgrix.Af3eFhiN4Wtrx8zKTU5PK8bAoUHAIyp8uNa	funcionario	t	2026-02-03 13:37:01.86638	2026-02-03 13:37:01.86638	1	1	\N	\N	\N	gestao	1966-02-01	2026-02-03 13:37:01.86638	\N	\N	\N	\N	\N	\N	\N	3	2026-02-04 15:23:30.074973	\N	funcionario_clinica	\N
25	52821297017	dfsiopi adpipoipo	uoiuoiu	oiuoiuoiu	oiuoiu@uiuooiu.com	$2a$10$PCXs6rTNkmAs7QKP/86XROJS2rnnc.Ma9guuHIYVl7KBomtA5RQ1m	funcionario	t	2026-02-03 13:03:58.168595	2026-02-03 13:03:58.168595	\N	\N	\N	\N	\N	operacional	2000-02-01	2026-02-03 13:03:58.168595	\N	\N	\N	\N	\N	\N	\N	7	2026-02-04 19:26:00.319216	1	funcionario_entidade	\N
\.
...
COPY public.laudos (id, lote_id, emissor_cpf, observacoes, status, criado_em, emitido_em, enviado_em, atualizado_em, relatorio_individual, relatorio_lote, relatorio_setor, hash_relatorio_individual, hash_relatorio_lote, hash_relatorio_setor, hash_pdf, job_id, arquivo_remoto_provider, arquivo_remoto_bucket, arquivo_remoto_key, arquivo_remoto_url) FROM stdin;
3	3	\N	\N	rascunho	2026-02-03 00:34:34.069447	\N	\N	2026-02-03 00:34:34.069447	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
4	4	\N	\N	rascunho	2026-02-03 01:48:59.773615	\N	\N	2026-02-03 01:48:59.773615	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
6	6	\N	\N	rascunho	2026-02-03 12:46:35.705728	\N	\N	2026-02-03 12:46:35.705728	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
8	8	\N	\N	rascunho	2026-02-03 13:06:10.778477	\N	\N	2026-02-03 13:06:10.778477	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
9	9	\N	\N	rascunho	2026-02-03 13:19:57.941205	\N	\N	2026-02-03 13:19:57.941205	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
10	10	\N	\N	rascunho	2026-02-03 13:37:18.014664	\N	\N	2026-02-03 13:37:18.014664	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
11	11	53051173991	\N	emitido	2026-02-03 17:48:30.403256	2026-02-03 17:48:30.403256	\N	2026-02-03 17:48:30.403256	\N	\N	\N	\N	\N	\N	f22fa1021dcf819feb5ab8a9b9357daa6f26c79e80234897e77d9e96436f5321	\N	\N	\N	\N	\N
20	20	\N	\N	\N	2026-02-04 14:38:11.685257	\N	\N	2026-02-04 14:38:11.685257	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
21	21	\N	\N	\N	2026-02-04 14:38:32.006823	\N	\N	2026-02-04 14:38:32.006823	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
1	1	\N	\N	emitido	2026-02-04 19:03:34.053286	\N	\N	2026-02-04 19:03:34.053286	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
2	2	\N	\N	emitido	2026-02-04 19:49:58.79625	\N	\N	2026-02-04 19:49:58.79625	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N	\N
\.
...
COPY public.lotes_avaliacao (id, clinica_id, empresa_id, descricao, tipo, status, liberado_por, liberado_em, criado_em, atualizado_em, laudo_enviado_em, finalizado_em, numero_ordem, contratante_id, emitido_em, enviado_em, hash_pdf, setor_id, processamento_em, modo_emergencia, motivo_emergencia) FROM stdin;
3	\N	\N	Lote 1 liberado para RELEGERE. Inclui 2 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	concluido	87545772920	2026-02-03 00:34:34.069447	2026-02-03 00:34:34.069447	2026-02-03 02:55:23.250202	\N	\N	1	1	\N	\N	\N	\N	\N	f	\N
4	\N	\N	Lote 2 liberado para RELEGERE. Inclui 2 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	concluido	87545772920	2026-02-03 01:48:59.773615	2026-02-03 01:48:59.773615	2026-02-03 12:13:26.499065	\N	\N	2	1	\N	\N	\N	\N	\N	f	\N
5	\N	\N	Lote 3 liberado para RELEGERE. Inclui 1 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	concluido	87545772920	2026-02-03 12:31:17.621704	2026-02-03 12:31:17.621704	2026-02-03 12:32:29.681589	\N	\N	3	1	\N	\N	\N	\N	\N	f	\N
6	1	1	Lote 1 liberado para Empresa CM01. Inclui 2 funcionário(s) elegíveis.	completo	concluido	04703084945	2026-02-03 12:46:34.745225	2026-02-03 12:46:34.745225	2026-02-03 12:50:41.326184	\N	\N	1	\N	\N	\N	\N	\N	\N	f	\N
7	\N	\N	Lote 4 liberado para RELEGERE. Inclui 1 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	ativo	87545772920	2026-02-03 13:04:20.446494	2026-02-03 13:04:20.446494	2026-02-03 13:04:20.446494	\N	\N	4	1	\N	\N	\N	\N	\N	f	\N
8	1	2	Lote 1 liberado para empresa cm 02. Inclui 2 funcionário(s) elegíveis.	completo	ativo	04703084945	2026-02-03 13:06:09.834137	2026-02-03 13:06:09.834137	2026-02-03 13:06:09.834137	\N	\N	1	\N	\N	\N	\N	\N	\N	f	\N
9	1	2	Lote 2 liberado para empresa cm 02. Inclui 2 funcionário(s) elegíveis.	completo	ativo	04703084945	2026-02-03 13:19:56.981956	2026-02-03 13:19:56.981956	2026-02-03 13:19:56.981956	\N	\N	2	\N	\N	\N	\N	\N	\N	f	\N
10	1	1	Lote 2 liberado para Empresa CM01. Inclui 1 funcionário(s) elegíveis.	completo	ativo	04703084945	2026-02-03 13:37:17.056032	2026-02-03 13:37:17.056032	2026-02-03 13:37:17.056032	\N	\N	2	\N	\N	\N	\N	\N	\N	f	\N
11	\N	\N	Lote 5 liberado para Staneley company . Inclui 2 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	concluido	08453792917	2026-02-03 17:37:21.99167	2026-02-03 17:37:21.99167	2026-02-03 17:48:30.403256	\N	\N	5	3	2026-02-03 17:48:30.403256+00	\N	\N	\N	\N	f	\N
21	\N	\N	Lote 6 liberado para RELEGERE. Inclui 1 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	ativo	87545772920	2026-02-04 14:38:32.006823	2026-02-04 14:38:32.006823	2026-02-04 14:38:32.006823	\N	\N	6	1	\N	\N	\N	\N	\N	f	\N
20	1	1	Lote 3 liberado para Empresa CM01. Inclui 1 funcionário(s) elegíveis.	completo	concluido	04703084945	2026-02-04 14:38:11.685257	2026-02-04 14:38:11.685257	2026-02-04 15:44:38.56882	\N	\N	3	\N	\N	\N	\N	\N	\N	f	\N
1	\N	\N	Lote 7 liberado para RELEGERE. Inclui 1 funcionário(s) elegíveis vinculados diretamente à entidade.	completo	ativo	87545772920	2026-02-04 19:03:34.053286	2026-02-04 19:03:34.053286	2026-02-04 19:03:34.053286	\N	\N	7	1	\N	\N	\N	\N	\N	f	\N
2	1	1	Lote 4 liberado para Empresa CM01. Inclui 1 funcionário(s) elegíveis.	completo	ativo	04703084945	2026-02-04 19:49:58.79625	2026-02-04 19:49:58.79625	2026-02-04 19:49:58.79625	\N	\N	4	\N	\N	\N	\N	\N	\N	f	\N
\.
...
395	16	1	Q1	75	2026-02-04 15:20:51.487196	1
396	16	1	Q2	100	2026-02-04 15:20:55.600863	2
397	16	1	Q3	50	2026-02-04 15:20:59.466912	3
398	16	1	Q9	75	2026-02-04 15:21:03.166351	9
399	16	2	Q13	50	2026-02-04 15:21:11.801208	13
400	16	2	Q17	25	2026-02-04 15:21:17.032216	17
401	16	2	Q18	75	2026-02-04 15:21:20.663697	18
402	16	2	Q19	100	2026-02-04 15:21:23.832674	19
403	16	3	Q20	100	2026-02-04 15:21:30.237565	20
404	16	3	Q21	0	2026-02-04 15:21:34.107219	21
405	16	3	Q23	25	2026-02-04 15:21:38.002684	23
406	16	3	Q25	50	2026-02-04 15:21:41.869433	25
407	16	3	Q26	75	2026-02-04 15:21:45.665847	26
408	16	3	Q28	100	2026-02-04 15:21:49.356984	28
409	16	4	Q31	75	2026-02-04 15:21:54.550489	31
410	16	4	Q32	50	2026-02-04 15:22:00.560766	32
411	16	4	Q33	100	2026-02-04 15:22:04.23529	33
412	16	4	Q34	75	2026-02-04 15:22:07.936979	34
413	16	5	Q35	50	2026-02-04 15:22:11.997533	35
414	16	5	Q38	75	2026-02-04 15:22:15.482974	38
415	16	5	Q41	100	2026-02-04 15:22:19.361134	41
416	16	6	Q43	50	2026-02-04 15:22:22.275545	43
417	16	6	Q45	25	2026-02-04 15:22:26.002004	45
418	16	7	Q48	0	2026-02-04 15:22:30.524596	48
419	16	7	Q52	25	2026-02-04 15:22:34.22775	52
420	16	7	Q55	50	2026-02-04 15:22:37.963517	55
421	16	8	Q56	75	2026-02-04 15:22:41.932489	56
422	16	8	Q57	100	2026-02-04 15:22:45.299686	57
423	16	8	Q58	75	2026-02-04 15:22:48.932982	58
424	16	9	Q59	0	2026-02-04 15:22:52.616674	59
425	16	9	Q61	25	2026-02-04 15:22:57.031705	61
426	16	9	Q62	50	2026-02-04 15:23:02.381662	62
427	16	9	Q64	75	2026-02-04 15:23:07.464322	64
428	16	10	Q65	100	2026-02-04 15:23:10.831351	65
429	16	10	Q66	75	2026-02-04 15:23:14.468906	66
430	16	10	Q68	50	2026-02-04 15:23:18.887317	68
431	16	10	Q70	100	2026-02-04 15:23:23.0125	70
432	17	1	Q1	75	2026-02-04 15:27:25.796519	1
433	17	1	Q2	100	2026-02-04 15:27:29.179618	2
434	17	1	Q3	75	2026-02-04 15:27:32.473714	3
435	17	1	Q9	50	2026-02-04 15:27:35.87719	9
436	17	2	Q13	75	2026-02-04 15:27:57.418987	13
437	17	2	Q17	75	2026-02-04 15:28:14.42692	17
438	17	2	Q18	100	2026-02-04 15:28:34.550714	18
439	17	2	Q19	50	2026-02-04 15:28:37.858556	19
440	17	3	Q20	50	2026-02-04 17:56:28.22649	20
441	17	3	Q21	75	2026-02-04 17:56:31.899967	21
442	17	3	Q23	100	2026-02-04 17:56:34.883133	23
443	17	3	Q25	75	2026-02-04 18:22:01.485379	25
444	17	3	Q26	100	2026-02-04 18:22:04.546571	26
445	17	3	Q28	75	2026-02-04 18:38:54.703544	28
446	17	4	Q31	50	2026-02-04 18:38:57.52613	31
447	17	4	Q32	50	2026-02-04 18:39:00.550799	32
448	17	4	Q33	100	2026-02-04 18:39:03.762219	33
449	17	4	Q34	50	2026-02-04 18:39:07.044548	34
450	17	5	Q35	50	2026-02-04 18:39:45.59343	35
451	17	5	Q38	75	2026-02-04 18:39:49.132435	38
452	17	5	Q41	75	2026-02-04 18:39:52.412794	41
453	17	6	Q43	75	2026-02-04 18:39:55.904395	43
454	17	6	Q45	50	2026-02-04 18:40:00.813988	45
455	17	7	Q48	50	2026-02-04 18:40:05.513985	48
456	17	7	Q52	75	2026-02-04 18:40:08.917451	52
457	17	7	Q55	75	2026-02-04 18:40:11.914045	55
458	17	8	Q56	50	2026-02-04 18:40:15.41854	56
459	17	8	Q57	25	2026-02-04 18:40:18.537436	57
461	17	9	Q59	25	2026-02-04 18:40:24.501683	59
462	17	9	Q61	75	2026-02-04 18:40:27.667579	61
463	17	9	Q62	75	2026-02-04 18:40:31.112016	62
464	17	9	Q64	50	2026-02-04 18:40:34.217185	64
465	17	10	Q65	50	2026-02-04 18:40:40.237487	65
466	17	10	Q66	75	2026-02-04 18:40:43.498637	66
467	17	10	Q68	75	2026-02-04 18:40:47.659213	68
468	17	10	Q70	75	2026-02-04 18:40:51.492323	70
460	17	8	Q58	100	2026-02-04 18:40:21.840907	58
473	18	1	Q1	50	2026-02-04 19:24:56.89366	1
474	18	1	Q2	100	2026-02-04 19:24:58.819717	2
475	18	1	Q3	75	2026-02-04 19:25:00.534292	3
476	18	1	Q9	50	2026-02-04 19:25:01.847505	9
477	18	2	Q13	25	2026-02-04 19:25:03.001323	13
478	18	2	Q17	50	2026-02-04 19:25:04.504228	17
479	18	2	Q18	75	2026-02-04 19:25:05.872694	18
480	18	2	Q19	100	2026-02-04 19:25:07.307599	19
481	18	3	Q20	75	2026-02-04 19:25:08.608694	20
482	18	3	Q21	50	2026-02-04 19:25:09.916189	21
483	18	3	Q23	25	2026-02-04 19:25:11.181406	23
484	18	3	Q25	0	2026-02-04 19:25:12.265475	25
485	18	3	Q26	25	2026-02-04 19:25:13.263029	26
486	18	3	Q28	25	2026-02-04 19:25:14.317607	28
487	18	4	Q31	25	2026-02-04 19:25:15.452005	31
488	18	4	Q32	25	2026-02-04 19:25:16.5059	32
489	18	4	Q33	50	2026-02-04 19:25:18.272113	33
490	18	4	Q34	50	2026-02-04 19:25:19.690428	34
491	18	5	Q35	50	2026-02-04 19:25:20.839969	35
492	18	5	Q38	75	2026-02-04 19:25:21.942238	38
493	18	5	Q41	50	2026-02-04 19:25:24.038877	41
494	18	6	Q43	50	2026-02-04 19:25:26.022363	43
495	18	6	Q45	50	2026-02-04 19:25:27.317892	45
496	18	7	Q48	50	2026-02-04 19:25:29.483105	48
497	18	7	Q52	50	2026-02-04 19:25:38.474085	52
498	18	7	Q55	50	2026-02-04 19:25:40.002356	55
499	18	8	Q56	100	2026-02-04 19:25:41.122005	56
500	18	8	Q57	75	2026-02-04 19:25:42.52032	57
501	18	8	Q58	75	2026-02-04 19:25:44.048791	58
502	18	9	Q59	50	2026-02-04 19:25:45.132037	59
503	18	9	Q61	0	2026-02-04 19:25:46.384249	61
504	18	9	Q62	25	2026-02-04 19:25:47.761683	62
505	18	9	Q64	50	2026-02-04 19:25:48.836547	64
506	18	10	Q65	75	2026-02-04 19:25:49.85746	65
507	18	10	Q66	100	2026-02-04 19:25:50.943445	66
508	18	10	Q68	75	2026-02-04 19:25:52.79956	68
509	18	10	Q70	75	2026-02-04 19:25:55.353061	70
\.
...
81	16	1	Demandas no Trabalho	75.00	alto	2026-02-04 15:23:27.252244
82	16	2	Organização e Conteúdo do Trabalho	62.50	medio	2026-02-04 15:23:27.516779
83	16	3	Relações Sociais e Liderança	58.33	medio	2026-02-04 15:23:27.749852
84	16	4	Interface Trabalho-Indivíduo	75.00	alto	2026-02-04 15:23:27.98257
85	16	5	Valores Organizacionais	75.00	alto	2026-02-04 15:23:28.214898
86	16	6	Traços de Personalidade	37.50	medio	2026-02-04 15:23:28.447024
87	16	7	Saúde e Bem-Estar	25.00	baixo	2026-02-04 15:23:28.680073
88	16	8	Comportamentos Ofensivos	83.33	alto	2026-02-04 15:23:28.912269
89	16	9	Comportamento de Jogo	37.50	medio	2026-02-04 15:23:29.144051
90	16	10	Endividamento Financeiro	81.25	alto	2026-02-04 15:23:29.377087
91	17	1	Demandas no Trabalho	75.00	alto	2026-02-04 18:40:55.23323
92	17	2	Organização e Conteúdo do Trabalho	75.00	alto	2026-02-04 18:40:55.48227
93	17	3	Relações Sociais e Liderança	79.17	alto	2026-02-04 18:40:55.723782
94	17	4	Interface Trabalho-Indivíduo	62.50	medio	2026-02-04 18:40:55.958872
95	17	5	Valores Organizacionais	66.67	alto	2026-02-04 18:40:56.193701
96	17	6	Traços de Personalidade	62.50	medio	2026-02-04 18:40:56.42859
97	17	7	Saúde e Bem-Estar	66.67	alto	2026-02-04 18:40:56.664099
98	17	8	Comportamentos Ofensivos	58.33	medio	2026-02-04 18:40:56.90077
99	17	9	Comportamento de Jogo	56.25	medio	2026-02-04 18:40:57.136377
100	17	10	Endividamento Financeiro	68.75	alto	2026-02-04 18:40:57.372179
111	18	1	Demandas no Trabalho	68.75	alto	2026-02-04 19:25:57.471957
112	18	2	Organização e Conteúdo do Trabalho	62.50	medio	2026-02-04 19:25:57.710535
113	18	3	Relações Sociais e Liderança	33.33	medio	2026-02-04 19:25:57.947019
114	18	4	Interface Trabalho-Indivíduo	37.50	medio	2026-02-04 19:25:58.182433
115	18	5	Valores Organizacionais	58.33	medio	2026-02-04 19:25:58.419392
116	18	6	Traços de Personalidade	50.00	medio	2026-02-04 19:25:58.65616
117	18	7	Saúde e Bem-Estar	50.00	medio	2026-02-04 19:25:58.893526
118	18	8	Comportamentos Ofensivos	83.33	alto	2026-02-04 19:25:59.129606
119	18	9	Comportamento de Jogo	31.25	baixo	2026-02-04 19:25:59.365338
120	18	10	Endividamento Financeiro	81.25	alto	2026-02-04 19:25:59.60196
\.
...
SELECT pg_catalog.setval('public.audit_logs_id_seq', 344, true);
...
SELECT pg_catalog.setval('public.auditoria_id_seq', 34, true);
...
SELECT pg_catalog.setval('public.auditoria_laudos_id_seq', 4, true);
...
SELECT pg_catalog.setval('public.avaliacoes_id_seq', 19, true);
...
SELECT pg_catalog.setval('public.lotes_avaliacao_id_seq', 21, true);
...
SELECT pg_catalog.setval('public.respostas_id_seq', 509, true);
...
SELECT pg_catalog.setval('public.resultados_id_seq', 120, true);
...
CREATE INDEX idx_audit_logs_resource_id ON public.audit_logs USING btree (resource, resource_id) WHERE (resource_id IS NOT NULL);
...
CREATE INDEX idx_auditoria_laudos_lote_acao ON public.auditoria_laudos USING btree (lote_id, acao) WHERE ((acao)::text = 'solicitar_emissao'::text);
...
--
-- Name: idx_lotes_avaliacao_status; Type: INDEX; Schema: public; Owner: neondb_owner
--

CREATE INDEX idx_lotes_avaliacao_status ON public.lotes_avaliacao USING btree (status) WHERE ((status)::text <> 'cancelado'::text);
...
--
-- Name: contratantes tr_contratantes_sync_status_ativa_robust; Type: TRIGGER; Schema: public; Owner: neondb_owner
--

CREATE TRIGGER tr_contratantes_sync_status_ativa_robust BEFORE INSERT OR UPDATE ON public.contratantes FOR EACH ROW EXECUTE FUNCTION public.contratantes_sync_status_ativa_robust();
...
CREATE TRIGGER trg_audit_lote_status AFTER UPDATE ON public.lotes_avaliacao FOR EACH ROW WHEN (((old.status)::text IS DISTINCT FROM (new.status)::text)) EXECUTE FUNCTION public.audit_lote_status_change();
...
CREATE TRIGGER trg_protect_lote_after_emit BEFORE UPDATE ON public.lotes_avaliacao FOR EACH ROW EXECUTE FUNCTION public.prevent_modification_lote_when_laudo_emitted();
...
CREATE TRIGGER trg_validar_transicao_status_lote BEFORE UPDATE ON public.lotes_avaliacao FOR EACH ROW WHEN (((old.status)::text IS DISTINCT FROM (new.status)::text)) EXECUTE FUNCTION public.fn_validar_transicao_status_lote();
...
CREATE TRIGGER trigger_prevent_avaliacao_mutation_during_emission BEFORE DELETE OR UPDATE ON public.avaliacoes FOR EACH ROW EXECUTE FUNCTION public.prevent_mutation_during_emission();
...
--
-- Name: empresas_clientes empresas_clientes_contratante_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: neondb_owner
--

ALTER TABLE ONLY public.empresas_clientes
    ADD CONSTRAINT empresas_clientes_contratante_id_fkey FOREIGN KEY (contratante_id) REFERENCES public.contratantes(id);
...
--
-- Name: TABLE equipe_administrativa; Type: ACL; Schema: public; Owner: neondb_owner
--
...
--
-- Name: TABLE funcionarios_operacionais; Type: ACL; Schema: public; Owner: neondb_owner
--
...
--
-- Name: TABLE gestores; Type: ACL; Schema: public; Owner: neondb_owner
--
...
--
-- Name: TABLE usuarios_resumo; Type: ACL; Schema: public; Owner: neondb_owner
--
