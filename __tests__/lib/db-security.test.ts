...
jest.mock('@/lib/db', () => ({
  ...
  transaction: async (cb: (client: any) => Promise<any>) =>
    cb({ query: (...args: any[]) => mockQuery(...args) }),
}));
...
describe('db-security.queryWithContext', () => {
  ...
  it('deve setar app.current_user_clinica_id quando perfil for rh', async () => {
    ...
    mockQuery.mockImplementation((sql: string, _params?: any[]) => {
      if (sql.includes('DISTINCT ec.clinica_id')) {
    ...
    expect(
      mockQuery.mock.calls.some(
        (c) =>
          c[0].includes('set_config') && c[1][0] === 'app.current_clinica_id'
      )
    ).toBe(true);
  });
  ...
  it('deve setar app.current_user_tomador_id quando perfil for gestor', async () => {
    ...
    mockQuery.mockImplementation((sql: string, _params?: any[]) => {
      if (sql.includes('DISTINCT fe.entidade_id')) {
        return Promise.resolve({
          rows: [{ entidade_id: '42' }],
          rowCount: 1,
        });
    ...
    expect(
      mockQuery.mock.calls.some(
        (c) =>
          c[0].includes('set_config') && c[1][0] === 'app.current_entidade_id'
      )
    ).toBe(true);
  });
});
