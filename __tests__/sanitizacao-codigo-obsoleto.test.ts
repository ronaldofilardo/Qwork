...
describe('Sanitização: Remoção de Código Obsoleto', () => {
    ...
    endpointsLaudos.forEach((filePath) => {
      it(`${filePath} não deve ter UPDATE laudos SET após emissão`, () => {
        ...
        if (fs.existsSync(fullPath)) {
          ...
          // Verificar se tem UPDATE laudos - se tiver, deve ter WHERE status = 'pendente'
          if (content.includes('UPDATE laudos')) {
            ...
            if (updateMatches) {
              updateMatches.forEach((update) => {
                // Se é UPDATE de hash ou atualizado_em, não deve existir
                if (
                  update.includes('hash_pdf') ||
                  update.includes('atualizado_em')
                ) {
                  fail(`UPDATE em laudo encontrado em ${filePath}: ${update}`);
                }
              });
            }
          }
  ...
  // ============================================================================
  // SANITIZAÇÃO: Migrations obsoletas
  // ============================================================================
  describe('Migrations: Aplicadas e não revertidas', () => {
  ...
  describe('Testes: Sem referências a código removido', () => {
    it('não deve ter testes para arquivo_pdf', () => {
      ...
      testFiles.forEach((testFile) => {
        if (testFile.endsWith('.test.ts') || testFile.endsWith('.test.tsx')) {
          ...
          // Se menciona arquivo_pdf, deve ser apenas em comentários de remoção
          if (content.includes('arquivo_pdf')) {
            expect(content).toMatch(/arquivo_pdf.*removid[oa]/i);
          }
        }
      });
    });
