...
describe('Integridade do Hash SHA-256 do Laudo', () => {
  ...
  beforeAll(async () => {
    ...
    await query("DELETE FROM clinicas WHERE cnpj = '11111111000199'", []);
  ...
  test('Hash do PDF deve ser calculado corretamente após geração', async () => {
    ...
    const laudoResult = await query(
      'SELECT status, emitido_em FROM laudos WHERE id = $1',
      [laudoId]
    );
    expect(laudoResult.rows[0].status).toBe('enviado');
  ...
  test('Quando PDF já existe mas hash no DB é nulo, deve recalcular e persistir o hash ao revalidar', async () => {
    ...
    await query('UPDATE laudos SET hash_pdf = NULL WHERE id = $1', [laudoId]);
    ...
    // Verificar DB
    const res = await query('SELECT hash_pdf FROM laudos WHERE id = $1', [
      laudoId,
    ]);
  ...
  test('Laudo deve ter status enviado após geração automática', async () => {
    ...
    expect(laudoResult.rows[0].status).toBe('enviado');
    expect(laudoResult.rows[0].emitido_em).not.toBeNull();
    expect(laudoResult.rows[0].enviado_em).not.toBeNull(); // Status canonizado para 'enviado' imediatamente
