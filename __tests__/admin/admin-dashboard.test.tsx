...
describe('AdminPage - Dashboard de Administração', () => {
  ...
  it('deve renderizar dashboard com abas de Clínicas e Emissores', async () => {
    ...
    // Clica na seção Contratantes para expandir
    const contratantesButton = screen.getByRole('button', { name: /contratantes/i })
    await user.click(contratantesButton)

    // Verifica se abas existem
    expect(screen.getByText('Contratantes')).toBeInTheDocument()
  ...
  it('deve fazer fetch dos dados das clínicas', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/admin/contratantes?tipo=clinica')
  ...
  it('deve permitir alternar entre abas Clínicas e Emissores', async () => {
    ...
    mockFetch.mockImplementation((url) => {
      ...
      if (url === '/api/admin/emissores') {
        return Promise.resolve(createMockResponse({ success: true, emissores: [] }))
      }
