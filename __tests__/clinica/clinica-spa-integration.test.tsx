...
describe('ClinicaSidebar Route-based Navigation', () => {
  ...
  it('renders sidebar with correct navigation structure', () => {
    ...
    expect(screen.getByText('Notificações')).toBeInTheDocument(); // Seção
  ...
  it('navigates to correct routes when clicking menu items', () => {
    ...
    // Testa navegação para Lotes
    fireEvent.click(screen.getByText('Lotes'));
    expect(mockPush).toHaveBeenCalledWith('/rh/lotes');
  ...
  it('displays counts correctly in badges', () => {
    ...
    expect(screen.getByText('3')).toBeInTheDocument(); // lotes
