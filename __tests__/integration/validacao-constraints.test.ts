...
describe('Validação de Constraints e Integridade do Banco', () => {
    ...
    it('deve permitir criar gestor COM entidade_id e SEM clinica_id', async () => {
      ...
      const contratante = await query(
        `INSERT INTO contratantes (
          ...
          'Resp', '11111111111', $4, '11988888888', true, true
        ) RETURNING id`,
        [
      ...
      const result = await query(
        ...
        [gestorCpf, `gestor${timestamp}@teste.com`, contratanteId]
      );

      expect(result.rows.length).toBe(1);
      expect(result.rows[0].tipo_usuario).toBe('gestor');
      expect(result.rows[0].entidade_id).toBe(contratanteId);
  ...
  describe('2. Constraint: funcionarios_owner_check', () => {
    ...
    it('deve permitir funcionário de entidade: contratante_id preenchido, clinica_id e empresa_id NULL', async () => {
      const timestamp = Date.now();
      ...
      const result = await query(
        ...
        RETURNING cpf, contratante_id, clinica_id, empresa_id`,
        [funcCpf, `func${timestamp}@teste.com`, contratanteId]
      );
    ...
    it('deve permitir funcionário de empresa: empresa_id E clinica_id preenchidos, contratante_id NULL', async () => {
      const timestamp = Date.now();

      // Criar estrutura clínica
      const contratante = await query(
        `INSERT INTO contratantes (
          tipo, nome, cnpj, email, telefone, endereco, cidade, estado, cep,
          responsavel_nome, responsavel_cpf, responsavel_email, responsavel_celular,
          ...
          'clinica', $1, $2, $3, '1199999999', 'Rua', 'SP', 'SP', '01234567',
          'Resp', '22222222222', $4, '11988888888', true, true
        ) RETURNING id`,
        [
      ...
      const clinica = await query(
        ...
        [contratante.rows[0].id, `Clínica ${timestamp}`]
      );
    ...
    it('deve FALHAR funcionário com contratante_id E clinica_id simultaneamente', async () => {
      const timestamp = Date.now();
  ...
  describe('5. Foreign Keys e Índices', () => {
    it('deve existir FK funcionarios → contratantes', async () => {
      const result = await query(
        `SELECT conname
         FROM pg_constraint
    ...
    it('deve existir índice em funcionarios.contratante_id', async () => {
      const result = await query(
        `SELECT indexname
         FROM pg_indexes
         WHERE tablename = 'funcionarios'
         AND indexdef LIKE '%contratante_id%'`
      );
  ...
  describe('7. Integridade Referencial', () => {
    it('todos os funcionários com contratante_id devem ter contratante existente', async () => {
      const result = await query(
    ...
    it('todos os lotes com contratante_id devem ter contratante existente', async () => {
      const result = await query(
        `SELECT COUNT(*) as orphans
