...
describe('Integração: cadastro -> contrato aceito -> iniciar pagamento', () => {
  ...
  beforeEach(async () => {
    ...
    const tomadorRes = await query(
      `INSERT INTO entidades (
        ...
        numero_funcionarios_estimado, plano_id, ativa, pagamento_confirmado, status
      ) VALUES (
        'entidade', 'tomador E2E', $1, 'e2e@teste.com', '11999999999', 'Rua E2E, 1', 'Cidade', 'ST', '00000000',
        '11122233344', 'Responsavel', 'resp@teste.com', '11911111111', 10, $2, false, false, 'aguardando_pagamento'
      ) RETURNING id`,
      [cnpj, planoId]
    );
  ...
  test('Deve iniciar pagamento usando contrato aceito e valor do contrato', async () => {
    ...
    const tomadorQuery = `SELECT c.id, c.nome, c.plano_id, c.status, COALESCE(c.numero_funcionarios_estimado, 1) as numero_funcionarios, 
         ...
         JOIN contratos ctr ON ctr.tomador_id = c.id AND ctr.id = $2
         WHERE c.id = $1`;

    const tomadorRes = await query(tomadorQuery, [
      tomadorId,
      contratoId,
    ]);
    ...
    const contratoValidate = await query(
      ...
      [contratoId, tomadorId]
    );

    // Tentar inserir pagamento diretamente para ver se há erro de constraint/trigger
    try {
      const insertRes = await query(
        `INSERT INTO pagamentos (tomador_id, contrato_id, valor, valor_por_funcionario, numero_funcionarios, status, metodo)
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
        [tomadorId, contratoId, 250.0, 25.0, 10, 'pendente', 'avista']
      );
    } catch (err: any) {
