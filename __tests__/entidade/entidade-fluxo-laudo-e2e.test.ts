...
describe('Fluxo E2E: Entidade → Lote → Emissor → Laudo', () => {
  ...
  describe('5. Geração de Laudo', () => {
    ...
    it('deve prevenir duplicação de laudo (constraint UNIQUE)', async () => {
      ...
      // Se o laudo está 'emitido', vamos marcá-lo como 'enviado' para este teste
      if (laudoExistente.rows[0].status === 'emitido') {
        await query(
          `UPDATE laudos SET status = 'enviado', enviado_em = NOW() WHERE lote_id = $1`,
          [loteId]
        );
      }
      expect(laudoExistente.rows[0].status).toMatch(/emitido|enviado/);
  ...
  describe('7. Validação de Integridade Final', () => {
    ...
    it('deve garantir que laudo foi criado e está vinculado ao lote', async () => {
      ...
      expect(result.rows.length).toBe(1);
      expect(result.rows[0].status).toMatch(/emitido|enviado/); // Pode estar emitido ou enviado
