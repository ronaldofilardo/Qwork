...
import { POST as emitirLaudo, PATCH as enviarLaudo } from '@/app/api/emissor/laudos/[loteId]/route'
...
describe('Funcionalidades de Hash SHA-256 e Envio de Laudos', () => {
  ...
  const mockHash = '03e0e0e365ea93a0cd56dcca77f4d449a7646d781d5c233116ac68ee9128dc19'
  ...
  describe('Geração de Hash SHA-256 na Emissão', () => {
    it('deve gerar e armazenar hash SHA-256 do PDF durante emissão', async () => {
      ...
      mockQuery.mockResolvedValueOnce({ rows: [{ titulo: 'Teste' }], rowCount: 1 })
      mockQuery.mockResolvedValueOnce({ rows: [{ total: 2, operacional: 1, gestao: 1 }], rowCount: 1 })
      mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 })
      mockQuery.mockResolvedValueOnce({ rows: [{ observacoes: null }], rowCount: 1 })
  ...
  describe('Validações de Integridade', () => {
    it('deve validar que hash corresponde ao PDF armazenado', async () => {
      ...
      const calculatedHash = crypto.createHash('sha256').update(storedPdf).digest('hex')
