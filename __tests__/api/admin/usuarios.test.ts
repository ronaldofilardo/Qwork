...
describe('admin usuários do sistema', () => {
  ...
  describe('PATCH /api/admin/usuarios/[id]', () => {
    it('deve inativar usuário ativo', async () => {
      ...
      mockQuery
        .mockResolvedValueOnce({
          rows: [{ id: 1, cpf: '11111111111', nome: 'Suporte 1', tipo_usuario: 'suporte', ativo: true }],
          rowCount: 1,
        } as any)
        .mockResolvedValueOnce({ rows: [], rowCount: 1 } as any);

      // Act
      const res = await PATCH(makePatchRequest({ ativo: false }), { params: { id: '1' } });
      const data = await res.json();
    ...
    it('deve ativar usuário inativo', async () => {
      ...
      mockQuery
        .mockResolvedValueOnce({
          rows: [{ id: 2, cpf: '22222222222', nome: 'Comercial 1', tipo_usuario: 'comercial', ativo: false }],
          rowCount: 1,
        } as any)
        .mockResolvedValueOnce({ rows: [], rowCount: 1 } as any);
    ...
    it('deve retornar 200 idempotente quando usuário já está inativo', async () => {
      ...
      mockQuery.mockResolvedValueOnce({
        rows: [{ id: 1, cpf: '11111111111', nome: 'Suporte 1', tipo_usuario: 'suporte', ativo: false }],
        rowCount: 1,
      } as any);
    ...
    it('deve retornar 400 para ID inválido (não numérico)', async () => {
      // Act
      const res = await PATCH(makePatchRequest({ ativo: false }), { params: { id: 'abc' } });
      const data = await res.json();
    ...
    it('deve retornar 404 quando usuário não existe', async () => {
      ...
      // Act
      const res = await PATCH(makePatchRequest({ ativo: false }), { params: { id: '999' } });
      const data = await res.json();
