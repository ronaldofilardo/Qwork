/**
 * @file __tests__/api/admin/representantes-busca.test.ts
 *
 * Testes para GET /api/admin/representantes/busca
 * Busca representantes por nome ou código para o modal de vínculo (admin e suporte).
 */
...
import { GET } from '@/app/api/admin/representantes/busca/route';

const mockQuery = query as jest.MockedFunction<typeof query>;

function makeReq(q?: string): NextRequest {
  const url = `http://localhost/api/admin/representantes/busca${q !== undefined ? `?q=${encodeURIComponent(q)}` : ''}`;
...
describe('GET /api/admin/representantes/busca', () => {
  ...
  it('aceita autenticação com perfil suporte', async () => {
    const { requireRole } = await import('@/lib/session');
