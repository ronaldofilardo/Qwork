...
describe('/api/admin/empresas/[id]/funcionarios/count', () => {
  ...
  describe('GET - Contar funcionários ativos', () => {
    it('deve retornar contagem correta de funcionários ativos', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/admin/empresas/1/funcionarios/count')
      const response = await getFuncionariosCount(request, { params: { id: '1' } })
    ...
    it('deve retornar erro para empresa não encontrada', async () => {
      ...
      mockQuery.mockClear()
      mockQuery
        .mockResolvedValueOnce({ rows: [], rowCount: 0 }) // empresa not found

      const request = new NextRequest('http://localhost:3000/api/admin/empresas/999/funcionarios/count')
      const response = await getFuncionariosCount(request, { params: { id: '999' } })

      // Como empresa não existe, deve retornar 404
      expect(response.status).toBe(404)
    })

    it('deve retornar erro para acesso negado', async () => {
      mockRequireRole.mockResolvedValue({
        cpf: '00000000000',
        nome: 'Admin Teste',
        perfil: 'admin'
      ...
      mockQuery
        .mockResolvedValueOnce({ rows: [{ clinica_id: 1 }], rowCount: 1 }) // admin lookup
        .mockResolvedValueOnce({ rows: [{ clinica_id: 2 }], rowCount: 1 }) // empresa de outra clínica
        .mockResolvedValueOnce({ rows: [], rowCount: 0 }) // admin check fails
...
describe('/api/admin/empresas/[id]/avaliacoes/pendentes/count', () => {
  ...
  describe('GET - Contar avaliações pendentes', () => {
    it('deve retornar contagem correta de avaliações pendentes', async () => {
      mockRequireRole.mockResolvedValue({
        cpf: '00000000000',
        nome: 'Admin Teste',
        perfil: 'admin'
      ...
      mockQuery
        .mockResolvedValueOnce({ rows: [{ clinica_id: 1 }], rowCount: 1 }) // empresa check
        .mockResolvedValueOnce({ rows: [{ clinica_id: 1 }], rowCount: 1 }) // admin check
        .mockResolvedValueOnce({ rows: [{ count: 6 }], rowCount: 1 }) // count result

      const request = new NextRequest('http://localhost:3000/api/admin/empresas/1/avaliacoes/pendentes/count')
      const response = await getAvaliacoesPendentesCount(request, { params: { id: '1' } })
      const data = await response.json()
    ...
    it('deve excluir avaliações concluídas e inativadas da contagem', async () => {
      ...
      expect(queryCall[0]).toContain("a.status NOT IN ('concluida', 'inativada')")
    })
