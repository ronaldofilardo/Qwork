...
const mockGetPayment = asaasClient.getPayment as jest.MockedFunction<typeof asaasClient.getPayment>;
...
describe('POST /api/admin/manutencao/pagamentos/[pagamentoId]/verificar', () => {
  ...
  it('deve retornar synced=false quando pagamento já está pago no banco', async () => {
    ...
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: 5, status: 'pago', asaas_payment_id: 'pay_123', valor: '250.00', nome: 'Entidade A' }],
      rowCount: 1,
    } as any);
  ...
  it('deve retornar synced=false quando não há asaas_payment_id', async () => {
    mockRequireRole.mockResolvedValueOnce({ cpf: '000', perfil: 'admin' } as any);
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: 5, status: 'aguardando_pagamento', asaas_payment_id: null, valor: '250.00', nome: 'Entidade A' }],
      rowCount: 1,
    } as any);
  ...
  it('deve retornar 502 quando Asaas lança erro', async () => {
    mockRequireRole.mockResolvedValueOnce({ cpf: '000', perfil: 'admin' } as any);
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: 5, status: 'aguardando_pagamento', asaas_payment_id: 'pay_err', valor: '250.00', nome: 'X' }],
      rowCount: 1,
    } as any);
  ...
  it('deve confirmar pagamento quando Asaas retorna CONFIRMED', async () => {
    ...
    mockQuery
      .mockResolvedValueOnce({
        rows: [{ id: 5, status: 'aguardando_pagamento', asaas_payment_id: 'pay_ok', valor: '250.00', nome: 'Entidade A' }],
        rowCount: 1,
      } as any)
  ...
  it('deve retornar synced=false quando Asaas retorna PENDING', async () => {
    mockRequireRole.mockResolvedValueOnce({ cpf: '000', perfil: 'admin' } as any);
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: 5, status: 'aguardando_pagamento', asaas_payment_id: 'pay_pend', valor: '250.00', nome: 'X' }],
      rowCount: 1,
    } as any);
