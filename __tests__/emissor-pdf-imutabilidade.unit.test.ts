...
describe('Endpoint Emissor PDF - Imutabilidade e Segurança', () => {
  ...
  describe('Auditoria: Logs com schema correto', () => {
    it('deve usar colunas em inglês no INSERT de audit_logs', () => {
      ...
      // Verificar se NÃO usa schema antigo em português
      expect(routeCode).not.toContain('acao');
      expect(routeCode).not.toContain('entidade');
      expect(routeCode).not.toContain('user_id');
    });
  });
