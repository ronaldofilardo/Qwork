...
test.describe('Integração E2E: Funcionário com RLS', () => {
  ...
  test('funcionário deve ver apenas suas próprias avaliações', async ({ page }) => {
  ...
  test('funcionário NÃO deve conseguir acessar dados de RH', async ({ page }) => {
...
test.describe('Integração E2E: Admin com RLS', () => {
  ...
  test('admin deve ver TODOS os funcionários', async ({ page }) => {
    await page.goto('/admin/funcionarios')
    
    // Verificar que a lista carregou
    await expect(page.locator('table')).toBeVisible()
    
    // Deve ver muitos funcionários (acesso total)
    const funcionarios = page.locator('tbody tr')
    const count = await funcionarios.count()
    expect(count).toBeGreaterThan(10) // Admin vê todos
  })

  test('admin deve poder gerenciar qualquer empresa', async ({ page }) => {
    await page.goto('/admin/empresas')
    
    // Verificar que pode editar empresa
    await page.locator('[data-testid="edit-empresa"]').first().click()
    await expect(page.locator('form')).toBeVisible()
  })
})

test.describe('Auditoria E2E', () => {
  test('ações devem ser registradas em audit_logs', async ({ page, request }) => {
    // Login como admin
    await page.goto('/login')
    await page.fill('input[name="cpf"]', '88888888888')
    await page.fill('input[name="senha"]', 'senha123')
