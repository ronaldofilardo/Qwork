...
describe('scripts/link-remote-laudos', () => {
  ...
  it('should link remote keys when found', async () => {
    db.query.mockResolvedValue({ rows: [{ laudo_id: 14, lote_id: 30 }], rowCount: 1 });
    fs.readFile.mockRejectedValue(new Error('ENOENT'));
    backblaze.findLatestLaudoForLote.mockResolvedValue('laudos/lote-30/laudo-123.pdf');
  ...
  it('should skip when no remote found', async () => {
    db.query.mockResolvedValue({ rows: [{ laudo_id: 15, lote_id: 31 }], rowCount: 1 });
    fs.readFile.mockRejectedValue(new Error('ENOENT'));
