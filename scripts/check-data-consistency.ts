...
async function checkDataConsistency() {
  ...
  perfilCheck.rows.forEach((row: any) => {
    const match = row.perfil === row.usuario_tipo || 
                  row.usuario_tipo.includes(row.perfil) ||
                  (row.perfil === 'funcionario' && row.usuario_tipo.startsWith('funcionario_'));
    const status = match ? '✅' : '❌';
    console.log(`   ${status} perfil="${row.perfil}" + usuario_tipo="${row.usuario_tipo}" (${row.total} registros)`);
  });
  ...
  if (missingData.rows.length > 0) {
    ...
    missingData.rows.forEach((row: any) => {
      console.log(`   - CPF ${row.cpf}: perfil="${row.perfil}", usuario_tipo="${row.usuario_tipo}"`);
    });
  } else {
  ...
  if (incompleteCheck.rows.length > 0) {
    ...
    incompleteCheck.rows.forEach((row: any) => {
      console.log(`   - Avaliação #${row.id} (CPF ${row.funcionario_cpf}): ${row.total_respostas} respostas, status="${row.status}"`);
    });
  } else {
    console.log('   ✅ Todas as avaliações com 37 respostas estão marcadas como concluídas');
  }
  ...
  loteStatusCheck.rows.forEach((row: any) => {
    ...
    const isCorrect = row.lote_status === expectedStatus || 
                      (row.lote_status === 'emitido' && shouldBeConcluido) ||
                      (row.lote_status === 'enviado' && shouldBeConcluido);
    
    const status = isCorrect ? '✅' : '⚠️';
    console.log(`   ${status} Lote #${row.id}: status="${row.lote_status}" | ${row.pendentes} pendentes, ${row.concluidas} concluídas, ${row.inativadas} inativadas`);
  });
  ...
  console.log('\n\n📊 RESUMO');
  console.log('=' .repeat(80));
  console.log('A análise acima mostra todos os pontos de inconsistência de dados.');
