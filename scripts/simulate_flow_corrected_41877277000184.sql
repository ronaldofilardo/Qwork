...
VALUES ((SELECT id FROM contratantes WHERE cnpj='41877277000184'), 5, 100, 2000, 'Contrato gerado automaticamente após pagamento', 'pendente_pagamento');

-- 3) Criar pagamento
INSERT INTO pagamentos (contratante_id, valor, valor_por_funcionario, numero_funcionarios, status, numero_parcelas, metodo)
VALUES ((SELECT id FROM contratantes WHERE cnpj='41877277000184'), 2000, 20, 100, 'pendente', 4, 'parcelado');
...
-- 5) Marcar pagamento confirmado e ativar contratante
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'criar_senha_inicial_entidade') THEN
    PERFORM criar_senha_inicial_entidade((SELECT id FROM contratantes WHERE cnpj='41877277000184'));
  END IF;
END;
$$;

-- 7) Inserir funcionário gestor_entidade usando senha gerada (se existir)
...
-- 8) Vincular funcionario ao contratante
...
SELECT id, numero_recibo, valor_parcela, tipo_recibo FROM recibos WHERE pagamento_id = (SELECT id FROM pagamentos WHERE contratante_id = (SELECT id FROM contratantes WHERE cnpj = '41877277000184')) ORDER BY id DESC LIMIT 1;
