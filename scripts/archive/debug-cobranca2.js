...
(async () => {
  ...
  try {
    const ctRes = await query(
      "SELECT id, cnpj, numero_funcionarios_estimado FROM contratantes WHERE regexp_replace(cnpj, '[^0-9]', '', 'g') = $1 LIMIT 1",
      [cnpj]
    );
    console.log('contratante:', ctRes.rows[0]);
    ...
    const contratos = await query(
      `SELECT id, plano_id, numero_funcionarios, numero_funcionarios_estimado, valor_personalizado, criado_em
       FROM contratos WHERE contratante_id = $1 ORDER BY criado_em DESC, id DESC LIMIT 10`,
      [ctId]
    );
    ...
    const contratosPlanos = await query(
      `SELECT id, plano_id, valor_personalizado_por_funcionario, numero_funcionarios_estimado, numero_funcionarios_atual, created_at
       FROM contratos_planos WHERE contratante_id = $1 ORDER BY created_at DESC, id DESC LIMIT 10`,
      [ctId]
    );
