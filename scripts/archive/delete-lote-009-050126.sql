-- Script: delete-lote-009-050126.sql
-- Objetivo: remover lote com código '009-050126' e todas as avaliações/respostas/laudos associados
-- Autor: gerado automaticamente (revise antes de executar)
-- Banco alvo: nr-bps_db (desenvolvimento)
-- USO: Rodar o arquivo em um cliente psql ou DBeaver. Primeiro execute o bloco de PREVIEW e o bloco DRY-RUN (ele faz ROLLBACK). Quando revisar e confirmar, execute o bloco de EXECUÇÃO REAL.

-- ===========================
-- 1) PREVIEW: listar lote(s) e registros relacionados
-- ===========================
SELECT la.id AS lote_id, la.status, la.liberado_por, la.clinica_id, la.empresa_id, la.liberado_em
...
FROM respostas r
WHERE r.avaliacao_id IN (SELECT id FROM avaliacoes WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126'));

SELECT COUNT(*) AS laudos_count
FROM laudos l
WHERE l.lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126');
...
CREATE TEMP TABLE backup_lote AS SELECT * FROM lotes_avaliacao WHERE codigo = '009-050126';
...
DELETE FROM respostas WHERE avaliacao_id IN (SELECT id FROM avaliacoes WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126'));
DELETE FROM resultados WHERE avaliacao_id IN (SELECT id FROM avaliacoes WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126'));
DELETE FROM auditoria_laudos WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126');
DELETE FROM laudos WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126');
DELETE FROM lotes_avaliacao_funcionarios WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126');
DELETE FROM avaliacoes WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126');
DELETE FROM lotes_avaliacao WHERE codigo = '009-050126';
...
SELECT 'POS_EXEC_Lotes' as objeto, COUNT(*) FROM lotes_avaliacao WHERE codigo = '009-050126'
UNION ALL SELECT 'POS_EXEC_Avaliacoes', COUNT(*) FROM avaliacoes WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126')
UNION ALL SELECT 'POS_EXEC_Respostas', COUNT(*) FROM respostas WHERE avaliacao_id IN (SELECT id FROM avaliacoes WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126'))
UNION ALL SELECT 'POS_EXEC_LAF', COUNT(*) FROM lotes_avaliacao_funcionarios WHERE lote_id IN (SELECT id FROM lotes_avaliacao WHERE codigo = '009-050126')
