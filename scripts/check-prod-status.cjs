...
async function checkProdStatus() {
  ...
  if (!prodUrl) {
    ...
    console.error('Uso:');
    console.error('  DATABASE_URL="postgresql://..." node scripts/check-prod-status.cjs');
    console.error('  OU');
  ...
  try {
    ...
    if (functionDef.rows.length === 0) {
      ...
      const usesDefault = def.includes('INSERT INTO laudos') && 
                         !def.includes('status') && 
                         !usesRascunho;
      ...
      if (usesRascunho) {
        ...
        console.log('⚠️  MIGRATION 1004 NÃO APLICADA: Função NÃO especifica status');
      ...
      const insertLineIndex = lines.findIndex(l => l.toLowerCase().includes('insert into laudos'));
    ...
    if (columnDefault.rows.length > 0) {
      ...
      if (col.column_default && col.column_default.includes('emitido')) {
        ...
        console.log('   Se a função não especificar status, será usado \'emitido\'');
        console.log('   Isso causa erro "Laudo não pode ser marcado como emitido sem hash_pdf"');
        console.log('\n💡 Solução:');
        console.log('   1. Aplicar Migration 1004 (função com status=\'rascunho\')');
        console.log('   2. OU alterar DEFAULT: ALTER TABLE laudos ALTER COLUMN status SET DEFAULT \'rascunho\';');
      } else if (col.column_default && col.column_default.includes('rascunho')) {
    ...
    constraints.rows.forEach(row => {
      ...
      if (row.constraint_name.includes('hash')) {
        console.log('   ↑ Esta constraint causa o erro se status=\'emitido\' sem hash_pdf');
      }
    });
    ...
    if (recentLaudos.rows.length === 0) {
      ...
      console.log(`   Total: ${recentLaudos.rows.length} laudos mais recentes:\n`);
      recentLaudos.rows.forEach(row => {
        const statusIcon = row.status === 'rascunho' ? '✓' : 
                          row.status === 'emitido' && !row.tem_hash ? '⚠️' : 
                          '•';
        ...
        if (row.status === 'emitido' && !row.tem_hash) {
          console.log('      ⚠️  INCONSISTENTE: Status emitido mas sem hash_pdf!');
        }
      });
    }
    ...
    if (count > 0) {
      console.log(`   ⚠️  ENCONTRADOS ${count} laudos com status='emitido' mas sem hash_pdf`);
    ...
    const funcionUsaRascunho = functionDef.rows[0] && 
                               functionDef.rows[0].definition.includes("'rascunho'");
    const defaultEEmitido = columnDefault.rows[0] && 
    ...
    if (defaultEEmitido && funcionUsaRascunho) {
      ...
      console.log('   SQL: ALTER TABLE laudos ALTER COLUMN status SET DEFAULT \'rascunho\';');
      console.log('   Nota: Não é obrigatório se Migration 1004 foi aplicada');
    }

    if (count > 0) {
      console.log(`\n3. CORRIGIR ${count} laudos inconsistentes`);
      console.log('   Opção A: UPDATE laudos SET status=\'rascunho\' WHERE status=\'emitido\' AND hash_pdf IS NULL;');
