...
-- detecta id da entidade recém criada

COMMIT;

-- Obter id da entidade
\gset
SELECT id FROM entidades WHERE cnpj='41877277000184' LIMIT 1;

-- 2) Criar pagamento (ATENÇÃO: contratante_id mantido temporariamente em pagamentos)
...
VALUES ((SELECT id FROM entidades WHERE cnpj='41877277000184'), 2000, 20, 100, 'pendente', 4, 'parcelado', NOW()) RETURNING id;
...
-- Gestores devem existir apenas em 'usuarios', não em 'funcionarios'
...
SELECT id, numero_recibo, valor, tipo_recibo FROM recibos WHERE pagamento_id = (SELECT id FROM pagamentos WHERE contratante_id = (SELECT id FROM entidades WHERE cnpj = '41877277000184')) ORDER BY id DESC LIMIT 1;
