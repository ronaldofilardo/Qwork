...
(async () => {
  ...
  const vals = await c.query('SELECT DISTINCT tipo_usuario FROM usuarios ORDER BY tipo_usuario');
  console.log('tipo_usuario values in use:', vals.rows.map(r => r.tipo_usuario));

  // Verificar hierarquia_comercial para usuario_id=55 (comercial)
  const hc = await c.query(
    'SELECT * FROM hierarquia_comercial WHERE comercial_id = 55 OR vendedor_id = 55'
  ).catch(() => ({ rows: [] }));
  ...
  const vp = await c.query(
    'SELECT * FROM vendedores_perfil WHERE usuario_id IN (54, 55)'
  ).catch(() => ({ rows: [] }));
  console.log('vendedores_perfil for 54,55:', vp.rows);

  // Verificar aceites_termos_usuario
  const at = await c.query(
    'SELECT * FROM aceites_termos_usuario WHERE usuario_id IN (54, 55)'
  ).catch(() => ({ rows: [] }));
  ...
  console.log('hierarquia_comercial cols:', hcCols.rows.map(r => r.column_name));
  ...
  console.log('vendedores_perfil cols:', vpCols.rows.map(r => r.column_name));
