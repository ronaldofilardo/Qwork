...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      WHEN f.indice_avaliacao < p_numero_lote_atual - 1 THEN ('Ã ndice atrasado (faltou ' || (p_numero_lote_atual - 1 - f.indice_avaliacao)::TEXT || ' lote(s))')::VARCHAR(100)
    ...
    CASE
      ...
      WHEN f.indice_avaliacao < p_numero_lote_atual - 2 THEN 'CRÃ TICA'::VARCHAR(20)
...
COMMENT ON FUNCTION calcular_elegibilidade_lote_contratante (INTEGER, INTEGER) IS 'Calcula quais funcionários (sem empresa) devem ser incluídos no próximo lote para uma entidade (contratante)';