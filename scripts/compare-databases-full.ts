...
async function main() {
  ...
  process.env.DATABASE_URL = 'postgresql://postgres:postgres@localhost:5432/qwork_dev?sslmode=disable';
  ...
  for (const enumName of allEnumNames) {
    ...
    if (!devValues) {
      ...
      console.log(`   Valores: ${devValues.map(v => v.enumlabel).join(', ')}`);
      hasEnumDiff = true;
    } else {
  ...
  for (const tableName of commonTables.sort()) {
    ...
    const missingInProdCols = [...devColNames].filter(c => !prodColNames.has(c));
    const missingInDevCols = [...prodColNames].filter(c => !devColNames.has(c));
    ...
    if (missingInProdCols.length > 0) {
      ...
      console.log(`   ❌ Colunas em DEV mas NÃO em PROD: ${missingInProdCols.join(', ')}`);
      hasColumnDiff = true;
    }
    ...
    if (missingInDevCols.length > 0) {
      ...
      console.log(`   ❌ Colunas em PROD mas NÃO em DEV: ${missingInDevCols.join(', ')}`);
      hasColumnDiff = true;
    }
  ...
  console.log(`Constraints diferentes: ${hasConstraintDiff ? '❌ SIM' : '✅ NÃO'}`);
