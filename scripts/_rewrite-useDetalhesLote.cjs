// ── CJS rewrite: useDetalhesLote.ts ─── shared imports + useLoteFiltros ────
// This script rewrites the existing hook to use shared lib/lote modules
...
export { normalizeString, formatarData, getClassificacaoLabel, getClassificacaoStyle, getStatusBadgeInfo };
...
export function useDetalhesLote() {
  ...
  const [permissionErrorHint, setPermissionErrorHint] = useState<string | null>(null);

  // == Modal state ==
  const [modalInativar, setModalInativar] = useState<ModalInativarState | null>(null);
  const [modalResetar, setModalResetar] = useState<ModalResetarState | null>(null);
  const [modalEmissao, setModalEmissao] = useState<ModalEmissaoState | null>(null);
  ...
  const loadLoteData = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          setPermissionErrorHint(
            errorData.hint || 'Acesso negado. Verifique sua cl\\u00ednica.',
          );
          setLoading(false);
          return;
        }
      ...
      if (!data.success) {
        alert(\`Erro: \${data.error || 'Resposta inv\\u00e1lida do servidor'}\`);
        router.push(\`/rh/empresa/\${empresaId}\`);
        return;
      }
    ...
    const checkSessionAndLoad = async () => {
      try {
        ...
        if (sessionData.perfil !== 'rh' && sessionData.perfil !== 'admin') {
          alert('Acesso negado. Apenas usu\\u00e1rios RH podem acessar esta p\\u00e1gina.');
          router.push('/dashboard');
          return;
        }
        await loadLoteData();
      } catch (error) {
        console.error('Erro ao verificar sess\\u00e3o:', error);
        router.push('/login');
      }
    };
    checkSessionAndLoad();
  ...
  const gerarRelatorioLote = useCallback(async () => {
    if (!lote) return;
    if (!confirm(\`Gerar relat\\u00f3rio PDF do lote \${lote.id}?\`)) return;
    try {
      const response = await fetch(\`/api/rh/relatorio-lote-pdf?lote_id=\${loteId}\`);
      if (!response.ok) {
        const error = await response.json();
        alert('Erro ao gerar relat\\u00f3rio: ' + (error.error || 'Erro desconhecido'));
        return;
      }
      ...
      console.error('Erro ao gerar relat\\u00f3rio:', error);
      alert('Erro ao gerar relat\\u00f3rio: ' + (error instanceof Error ? error.message : 'Erro desconhecido'));
    }
  }, [lote, loteId]);

  const gerarRelatorioFuncionario = useCallback(
    async (cpf: string, nome: string) => {
      if (!confirm(\`Gerar relat\\u00f3rio PDF de \${nome}?\`)) return;
      try {
        const response = await fetch(
  ...
  const solicitarEmissao = useCallback(async () => {
    ...
    const confirmado = confirm(
      \`Confirma a solicita\\u00e7\\u00e3o de emiss\\u00e3o do laudo para o lote \${lote.id}?\\n\\nO laudo ser\\u00e1 gerado e enviado para o emissor respons\\u00e1vel.\`,
    );
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Erro ao solicitar emiss\\u00e3o');
      toast.success('Emiss\\u00e3o solicitada com sucesso!');
      ...
      toast.error(error.message || 'Erro ao solicitar emiss\\u00e3o');
    }
  }, [lote, loadLoteData]);
  ...
  const abrirModalInativar = useCallback(
    (avaliacaoId: number, nome: string, cpf: string) => {
      setModalInativar({ avaliacaoId, funcionarioNome: nome, funcionarioCpf: cpf });
    },
    [],
  );
...
console.log(
  'OK: useDetalhesLote.ts rewritten (' + content.split('\\n').length + ' lines)'
);
