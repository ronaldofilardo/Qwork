...
const { Client } = pg;
const tomadorId = process.argv[2] || '20';
...
(async () => {
  try {
    ...
    const r = await client.query(
      'SELECT id, cnpj, responsavel_cpf, responsavel_nome, responsavel_email, tipo FROM tomadors WHERE id = $1',
      [tomadorId]
    );
    if (r.rows.length === 0) {
      console.error('tomador not found', tomadorId);
      process.exit(1);
    }
    const c = r.rows[0];
    ...
    const exists = await client.query(
      'SELECT id FROM entidades_senhas WHERE tomador_id = $1 AND cpf = $2',
      [c.id, cpfParaUsar]
    );
    if (exists.rows.length > 0) {
      await client.query(
        'UPDATE entidades_senhas SET senha_hash = $1, atualizado_em = CURRENT_TIMESTAMP WHERE tomador_id = $2 AND cpf = $3',
    ...
    if (u.rows.length > 0) {
      ...
      await client.query(
        'UPDATE usuarios SET nome = $1, email = $2, tipo_usuario = $3, tomador_id = $4, ativo = true, senha_hash = $5, atualizado_em = CURRENT_TIMESTAMP WHERE id = $6',
        [
      ...
      const ins = await client.query(
        `INSERT INTO usuarios (cpf, nome, email, senha_hash, tipo_usuario, tomador_id, ativo, criado_em, atualizado_em)
