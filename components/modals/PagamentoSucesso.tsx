...
export function PagamentoSucesso({ confirmResponse, pagamentoId, onFechar }: PagamentoSucessoProps) {
  ...
  return (
    <div className="text-center py-8">
      ...
      {resp?.proximos_passos ? (
        <div className="mb-4 text-left">
          {resp.proximos_passos.map((p, i) => (
            <p key={i} className="text-gray-600 mb-1">{p}</p>
          ))}
        </div>
      ) : (
        <>
          <p className="text-gray-600 mb-1">Seu pagamento foi processado com sucesso.</p>
      ...
      {pagamentoId && (
        <p className="text-xs text-gray-400 mb-6">ID do Pagamento: #{pagamentoId}</p>
      )}
      <button
        onClick={onFechar}
