...
import { TipoContratante } from '@/lib/db';
            ...
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                etapaAtual === 'responsavel'
                  ? 'bg-orange-500 text-white'
                  : ['confirmacao'].includes(etapaAtual)
                    ? 'bg-green-500 text-white'
                    : 'bg-gray-300 text-gray-600'
              }`}
            >
              {['confirmacao'].includes(etapaAtual) ? <Check size={16} /> : '4'}
            </div>
            <div className="w-8 h-1 bg-gray-300">
              <div
                className={`h-full ${['confirmacao'].includes(etapaAtual) ? 'bg-orange-500' : ''}`}
              />
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          {sucesso ? (
            ...
            <>
              ...
              {/* Etapa 4: Contrato (removida — tratada pelo backend) */}
