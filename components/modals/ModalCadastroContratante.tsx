...
import DadosStep from './ModalCadastroContratante/DadosStep';
import ResponsavelStep from './ModalCadastroContratante/ResponsavelStep';
import ConfirmacaoStep from './ModalCadastroContratante/ConfirmacaoStep';
...
export default function ModalCadastroContratante({
  isOpen,
  onClose,
  tipo: tipoInicial,
}: ModalCadastroContratanteProps) {
  ...
  // Fetch e ajustes de plano são gerenciados pelo hook `useCadastroContratante` (internamente já faz getPlanos e ajuste de número de funcionários).

  // A geração de contrato foi extraída para `lib/cadastroContratante.ts` e deve ser
  // chamada via `gerarContratoSimulado({ plano, dadosContratante, dadosResponsavel, numeroFuncionarios, tipo })` quando necessário.
  ...
  // Validator logic moved to `lib/cadastroContratante` (use validarEtapaDados)

  // Validator logic moved to `lib/cadastroContratante` (use validarEtapaResponsavel)
