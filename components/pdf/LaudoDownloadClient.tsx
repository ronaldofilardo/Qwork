/**
 * Componente client-side para geração de PDFs de laudos
...
interface LaudoDownloadClientProps {
  htmlContent: string;
  filename?: string;
...
export function LaudoDownloadClient({
  htmlContent,
  filename = 'laudo',
  loteId,
  className = '',
}: LaudoDownloadClientProps) {
  ...
  const gerarPDF = async () => {
    ...
    try {
      // Criar iframe temporário com o HTML do laudo
      ...
      const finalFilename = `${filename}${loteId ? `-lote-${loteId}` : ''}-${timestamp}.pdf`;
  ...
  return (
    <div className={className}>
      <button
        ...
        className="bg-accent hover:bg-accent-hover text-white px-4 py-2 rounded disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
      >
        {isGenerating ? (
          <>
            <svg
