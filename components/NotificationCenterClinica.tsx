...
export default function NotificationCenterClinica({
  onNavigateToLote,
}: NotificationCenterClinicaProps) {
  ...
  useEffect(() => {
    ...
    const visualizadasStorage = localStorage.getItem(
      'notificacoes_visualizadas_clinica'
    );
          ...
          try {
            ...
            if (
              ...
              if (data.type === 'connected') {
                console.log(
                  '[SSE] Servidor confirmou conexão:',
                  data.timestamp
                );
              } else if (data.type === 'notifications') {
                console.log('[SSE] Notificações recebidas:', data.total);
                // Filtrar notificações não visualizadas
                const naoVisualizadas = (data.data || []).filter(
                  (notif: NotificacaoClinica) => !visualizadas.has(notif.id)
                );
    ...
    const handleRouteChange = () => {
      if (notificacoes.length > 0) {
        ...
        localStorage.setItem(
          'notificacoes_visualizadas_clinica',
          JSON.stringify([...novasVisualizadas])
        );
        setTotalNaoLidas(0);
      }
    };
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {notificacoes.map((notif) => (
                          <tr
                            key={notif.id}
                            onClick={() =>
                              handleNotificacaoClick(notif.lote_id)
                            }
                          ...
                          >
                            <td className="px-3 py-2 whitespace-nowrap">
                              <div className="flex items-center">
                                <div
                                  className={`flex-shrink-0 p-1.5 rounded-md shadow-sm ${getColorForTipo(notif.tipo)}`}
                                >
                            ...
                            <td className="px-3 py-2 whitespace-nowrap">
                              <div
                                className="text-xs text-gray-900 max-w-[120px] truncate"
                                title={notif.titulo}
                              >
                                {notif.titulo}
                              </div>
                            ...
                            <td
                              className="px-3 py-2 whitespace-nowrap text-xs text-gray-600 max-w-[120px] truncate"
