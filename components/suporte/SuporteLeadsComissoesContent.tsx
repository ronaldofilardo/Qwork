...
export function SuporteLeadsComissoesContent() {
  ...
  const carregar = useCallback(async (p: number, t: Tab) => {
    ...
    try {
      ...
      if (t === 'aprovacao_suporte')
        params.set('requer_aprovacao_suporte', 'true');
  ...
  const handleAprovar = async (id: number) => {
    setActionLoading(id);
    try {
      const res = await fetch(`/api/suporte/leads/${id}/aprovar`, {
        method: 'PATCH',
      });
      if (res.ok) {
        void carregar(page, tab);
    ...
    <div className="space-y-4">
      ...
      {tab === 'aprovacao_suporte' && (
        <div className="flex items-start gap-2 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 text-sm text-orange-800">
          ...
          <span>
            Leads abaixo requerem aprovação do Suporte — o valor negociado é
            inferior ao custo mínimo de operação.
          </span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border p-12 text-center">
          <p className="text-gray-500 text-sm">
            {tab === 'aprovacao_suporte'
              ? 'Nenhum lead aguardando aprovação do Suporte.'
